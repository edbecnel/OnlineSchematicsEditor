...
export function saveJSON(ctx) {
    ...
    const data = {
        ...
        netClasses: Object.fromEntries(Object.entries(ctx.NET_CLASSES)
...
export function loadFromJSON(ctx, text) {
    ...
    ctx.nets.clear();
    (data.nets || ['default']).forEach((n) => ctx.nets.add(n));
    if (!ctx.nets.has('default'))
        ctx.nets.add('default');
    ...
    ctx.wires.forEach((w) => {
        ...
        if (w.stroke.widthNm == null && typeof w.stroke.width === 'number') {
            w.stroke.widthNm = Math.round((w.stroke.width || 0) * 1000000); // NM_PER_MM constant
        }
        if (!w.netId)
            w.netId = 'default';
    });
...
export function installFileIOHandlers(ctx, saveBtn, loadBtn, fileInput) {
    // Save button
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        reader.onload = () => {
            try {
                loadFromJSON(ctx, reader.result);
            }
            catch (err) {
