 ...
 */
export function createPin(id, x, y, options) {
    return {
        id,
        x,
        y,
 ...
 */
export function createDIPComponent(pinCount, pinSpacing = 2 * GRID) {
    if (pinCount % 2 !== 0) {
    ...
    for (let i = 0; i < pinsPerSide; i++) {
        ...
        pins.push(createPin(String(i + 1), -bodyWidth / 2 - pinLength, y, { electricalType: 'passive', name: String(i + 1) }));
    }
    // Right side pins (bottom to top)
    for (let i = 0; i < pinsPerSide; i++) {
        const y = (bodyHeight / 2) - (i * pinSpacing) - (1.5 * GRID);
        pins.push(createPin(String(pinsPerSide + i + 1), bodyWidth / 2 + pinLength, y, { electricalType: 'passive', name: String(pinsPerSide + i + 1) }));
    }
    // Body rectangle
    graphics.push({
        type: 'rectangle',
 ...
 */
export function createCustomComponent(id, type, x, y, template, options) {
    return {
        id,
