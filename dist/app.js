...
(function () {
    ...
    const polarInputGroup = $q('#polarInputGroup');
    const coordInputLength = $q('#coordInputLength');
    const coordInputAngle = $q('#coordInputAngle');
    let toastContainer = null;
    function ensureToastContainer() {
        if (toastContainer)
            return toastContainer;
        let el = document.getElementById('editorToastContainer');
        if (!el) {
    ...
    function readStoredLibraryText(libraryName) {
        try {
    ...
    function writeStoredLibraryText(libraryName, text) {
        try {
    ...
    function removeStoredLibraryText(libraryName) {
        try {
    ...
    function clearSymbolLibraryCache(libraryName) {
        if (libraryName) {
    ...
    const libraryTextCache = new Map();
    let pendingLibrarySelection = null;
    let pendingLibrarySymbol = null;
    ...
    if (typeof window !== 'undefined') {
        const globals = window.OSEditor || (window.OSEditor = {});
        globals.clearSymbolLibraryCache = (libraryName) => clearSymbolLibraryCache(libraryName);
    }
    function clearLibraryPlacementState() {
        pendingLibrarySelection = null;
        pendingLibrarySymbol = null;
    }
    function inferPlaceType(libraryName, symbolName) {
        const libLc = libraryName.toLowerCase();
        const nameLc = symbolName.toLowerCase();
        const combined = `${libLc} ${nameLc}`;
        if (combined.includes('diode') || combined.includes('rectifier') || combined.includes('tvs'))
            return 'diode';
        if (combined.includes('capacitor'))
            return 'capacitor';
        if (combined.includes('inductor') || combined.includes('coil'))
            return 'inductor';
        if (combined.includes('ground') || combined.includes('earth'))
            return 'ground';
        if (combined.includes('battery') || combined.includes('cell'))
            return 'battery';
        if (combined.includes('ac') || combined.includes('sine'))
            return 'ac';
        if (combined.includes('transistor') || combined.includes('bjt') || combined.includes('npn'))
            return 'npn';
        if (combined.includes('pnp'))
            return 'pnp';
        return 'resistor';
    }
    ...
    function cloneGraphicElement(el) {
        switch (el.type) {
            case 'polyline':
            case 'polygon':
                return {
    ...
    function clonePropertyData(prop) {
        return {
            value: prop.value,
    ...
    function clonePropertyMap(source) {
        const result = {};
    ...
    function buildComponentForPlacement(type, position, rot) {
    ...
    function removeSymbolLibrary(libraryName) {
        if (!libraryName)
            return;
        if (!Object.prototype.hasOwnProperty.call(projectSettings.symbolLibraries, libraryName))
            return;
        Inspector.clearLibraryCollapseState(libraryName);
        const nextIndex = { ...projectSettings.symbolLibraries };
    ...
    function handleLibrarySymbolSelect(libraryName, symbolName) {
    ...
    let kicadFirstSegmentPerpLock = null;
    ...
    function dbg(tag, data) {
        ...
        try {
            if (data !== undefined)
                console.log(`[moves] ${tag}`, data);
            else
                console.log(`[moves] ${tag}`);
        }
        catch (_) { }
    }
    ...
    function redrawGrid() {
        ...
        if (pat) {
            ...
            while (pat.firstChild)
                pat.removeChild(pat.firstChild);
            const bg = document.createElementNS(ns, 'rect');
            bg.setAttribute('x', '0');
            bg.setAttribute('y', '0');
            bg.setAttribute('width', String(majorUser));
            bg.setAttribute('height', String(majorUser));
            ...
            for (let xi = 0; xi <= cellsPerMajor; xi++) {
                ...
                ln.setAttribute('y2', String(majorUser));
            ...
            for (let yi = 0; yi <= cellsPerMajor; yi++) {
                ...
                ln.setAttribute('x2', String(majorUser));
        ...
        if (patBold) {
            ...
            while (patBold.firstChild)
                patBold.removeChild(patBold.firstChild);
            const rect = document.createElementNS(ns, 'rect');
            rect.setAttribute('width', String(majorUser));
            rect.setAttribute('height', String(majorUser));
            rect.setAttribute('fill', 'url(#grid)');
            patBold.appendChild(rect);
            const border = document.createElementNS(ns, 'path');
            border.setAttribute('d', `M${majorUser} 0H0V${majorUser}`);
            border.setAttribute('fill', 'none');
            border.setAttribute('stroke', 'var(--grid-bold)');
    ...
    let nets = new Set(['default']);
    let activeNetClass = 'default';
    let undoStack = [];
    let redoStack = [];
     ...
     */
    function createLabelsForComponent(comp) {
    ...
    const defaultWireColor = (getComputedStyle(document.documentElement).getPropertyValue('--wire').trim() || '#008000');
            ...
            if (netName !== 'default') {
                ...
                delBtn.onclick = () => {
                    if (confirm(`Delete net "${netName}"? Wires using this net will be assigned to "default".`)) {
                        ...
                        wires.forEach(w => { if (w.netId === netName)
                            w.netId = 'default'; });
                        renderNetList();
                        redraw();
                    }
    ...
    function setMode(m) {
        ...
        if (m === 'move') {
            ...
            try {
                requestAnimationFrame(() => ensureFinishSwpMove());
            }
            catch (_) {
                setTimeout(() => ensureFinishSwpMove(), 0);
            }
        }
    ...
    (function attachModeButtonsFinalizeGuard() {
        try {
            ...
            document.addEventListener('click', (e) => {
                ...
                const btn = target ? target.closest('button[data-mode]') : null;
                const nextMode = btn?.dataset?.mode;
    ...
    (function attachLightBgColorPicker() {
        ...
        function applyLightBgColor(color, updatePicker = true) {
            ...
            const selectedSwatch = Array.from(swatches).find(s => s.getAttribute('data-color')?.toLowerCase() === color.toLowerCase());
        ...
        colorPicker.addEventListener('change', (e) => {
            ...
            const swatches = swatchContainer.querySelectorAll('.color-swatch');
            const existingSwatch = Array.from(swatches).find(s => s.getAttribute('data-color')?.toLowerCase() === color.toLowerCase());
    ...
    function drawComponent(c) {
        ...
        setAttr(hit, 'x', c.x - 40);
        setAttr(hit, 'y', c.y - 40);
        setAttr(hit, 'width', 80);
        setAttr(hit, 'height', 80);
        ...
        g.addEventListener('pointermove', (e) => {
            if (!dragging || mode !== 'move')
                return;
            const p = svgPoint(e);
            const shiftHeld = e.shiftKey; // Detect Shift key for temporary constraint bypass
            ...
            if (componentDragState && componentDragState.wires.length > 0) {
                ...
                for (const dragWire of componentDragState.wires) {
                    ...
                    const ghostWirePoints = dragWire.originalPoints.map(pt => {
                        if (dragWire.axis === 'x') {
        ...
        if (labelText) {
            labelText.addEventListener('pointerdown', (e) => {
                ...
                if (typeof labelText.setPointerCapture === 'function' && e.isPrimary) {
                    try {
                        labelText.setPointerCapture(e.pointerId);
                    }
                    catch (_) { }
                }
            });
            ...
            labelText.addEventListener('pointerup', (e) => {
                if (textDragState && textDragState.kind === 'label' && textDragState.componentId === c.id) {
                    if (typeof labelText.releasePointerCapture === 'function' && e.isPrimary) {
                        try {
                            labelText.releasePointerCapture(e.pointerId);
                        }
                        catch (_) { }
                    }
                    textDragState = null;
        ...
        if (valueText) {
            valueText.addEventListener('pointerdown', (e) => {
                ...
                if (typeof valueText.setPointerCapture === 'function' && e.isPrimary) {
                    try {
                        valueText.setPointerCapture(e.pointerId);
                    }
                    catch (_) { }
                }
            });
            ...
            valueText.addEventListener('pointerup', (e) => {
                if (textDragState && textDragState.kind === 'value' && textDragState.componentId === c.id) {
                    if (typeof valueText.releasePointerCapture === 'function' && e.isPrimary) {
                        try {
                            valueText.releasePointerCapture(e.pointerId);
                        }
                        catch (_) { }
                    }
                    textDragState = null;
    ...
    function finalizeEmbeddedMove(source) {
        ...
        if (sel && sel.kind === 'component')
            comp = components.find(x => x.id === sel.id);
        if (!comp) {
            comp = components.find(x => x.embeddedInWireId);
        }
        if (!comp)
            return;
    ...
    function redrawCanvasOnly() {
                ...
                if (mode === 'delete') {
                    ...
                    if (mode === 'select' || mode === 'move') {
                        ...
                        if (mode === 'move' && currentSel && currentSel.kind === 'wire' && currentSel.id === w.id) {
                            ...
                            const stretchKind = (isHorizontal || isVertical) ? 'orthogonal' : 'free';
                            ...
                            const findStubHost = (pt, extraExcludeWireIds, includeSegmentEndpoints = false) => {
                                const EPS = 0.01;
                                const tol = 1.0;
                                let best = null;
                                for (const ww of wires) {
                                    if (ww.id === w.id)
                                        continue;
                                    if (extraExcludeWireIds && extraExcludeWireIds.has(ww.id))
                                        continue;
                                    for (let si = 0; si < ww.points.length - 1; si++) {
                                        ...
                                        if (isVert) {
                                            ...
                                            if (!best || d < best.d)
                                                best = { hostWireId: ww.id, hostSegIndex: si, d, hostT, hostPt };
                                        }
                                        else if (isHoriz) {
                                            ...
                                            const { proj, t } = Geometry.projectPointToSegmentWithT(pt, a, b);
                                            if (!includeSegmentEndpoints && (t <= 0 || t >= 1))
                                                continue;
                            ...
                            if (stretchKind === 'free') {
                                ...
                                const rubberBandExclude = new Set();
                                const markRubberBandAt = (pt) => {
                                    const tol = 1.0;
                                    for (const ww of wires) {
                                        if (ww.id === w.id)
                                            continue;
                            ...
                            const connectedAtStart = hasJunctionAtStart ? [] : wires.filter(wire => {
                                if (wire.id === w.id)
                                    return false;
                                if (stretchKind === 'free' && excludeHostAtStart.has(wire.id))
                                    return false;
                            ...
                            const connectedAtEnd = hasJunctionAtEnd ? [] : wires.filter(wire => {
                                if (wire.id === w.id)
                                    return false;
                                if (stretchKind === 'free' && excludeHostAtEnd.has(wire.id))
                                    return false;
                                const wireStart = wire.points[0];
                                const wireEnd = wire.points[wire.points.length - 1];
                            ...
                            const wireAxis = isHorizontal ? 'x' : 'y';
                            ...
                            const slidingRubberBands = [];
                            if (stretchKind === 'free') {
                                const considerConn = (which, conn) => {
                                ...
                                for (const conn of connectedAtStart)
                                    considerConn('start', conn);
                                for (const conn of connectedAtEnd)
                                    considerConn('end', conn);
                            }
        ...
        if (showJunctionDots) {
            const isKicadMode = window.routingKernelMode === 'kicad';
            const suppressedKeys = new Set(junctions
                .filter(j => j.suppressed)
                .map(j => Geometry.keyPt({ x: Math.round(j.at.x), y: Math.round(j.at.y) })));
            const manualKeys = new Set(junctions
    ...
    function removeJunctionsAtWireEndpoints(w) {
        if (!w.points || w.points.length < 2)
            return;
        const EPS = 1e-3;
        ...
        junctions = junctions.filter(j => {
            ...
            if (!atFirst && !atLast)
                return true; // Keep junction if not at this wire's endpoints
            // Check if any OTHER wires are connected to this junction point
            const jPt = j.at;
            const otherWiresConnected = wires.some(other => {
                if (other.id === w.id)
                    return false; // Skip the wire being deleted
                return other.points.some(pt => Math.abs(pt.x - jPt.x) < EPS && Math.abs(pt.y - jPt.y) < EPS);
            });
    ...
    function nearestAnchorTo(pt, thresholdPx = 10) {
        ...
        for (const a of anchors) {
            ...
            if (minAxisDist < bestD) {
                bestD = minAxisDist;
                best = a;
            }
        }
        if (bestD <= thresholdPx)
            return best;
        return null;
    }
    ...
    function nearestAnchorToEuclid(pt, thresholdPx = 10) {
        const anchors = collectAnchors();
        const scale = userScale();
        let best = null;
        let bestD = Infinity;
    ...
    function debugDumpAnchors() {
        try {
            ...
            const rects = $qa('[data-endpoint]', gOverlay).map(r => ({
                endpoint: r.endpoint || null,
                wireId: r.wireId || null,
                bbox: r.getBBox()
            }));
        }
        catch (err) { }
     ...
     */
    function snapPointForTJunction(p, thresholdPx = 10) {
        const scale = userScale();
    ...
    function wireEndsAt(w, pt) {
        if (Geometry.eqPtEps(w.points[0], pt, 1))
            return 0;
        if (Geometry.eqPtEps(w.points[w.points.length - 1], pt, 1))
    ...
    function adjacentOther(w, endPt) {
        ...
        if (n < 2)
            return null;
        if (Geometry.eqPtEps(w.points[0], endPt, 1))
            return w.points[1];
        if (Geometry.eqPtEps(w.points[n - 1], endPt, 1))
            return w.points[n - 2];
        return null;
    }
    ...
    svg.addEventListener('pointerdown', (e) => {
        ...
        let endpointClickedWireId = null;
        let endpointClickedWireEndIndex = null;
        if (tgt && (tgt.tagName === 'rect' || tgt.tagName === 'circle') && tgt.endpoint) {
            endpointClicked = tgt.endpoint;
            if (typeof tgt.wireId === 'string') {
                endpointClickedWireId = tgt.wireId;
            }
            if (typeof tgt.endpointIndex === 'number') {
                endpointClickedWireEndIndex = tgt.endpointIndex;
            }
        }
            ...
            if (!existingJunction || !existingJunction.manual || existingJunction.suppressed) {
                ...
                for (const w of wires) {
                    ...
                    for (let i = 0; i < w.points.length - 1; i++) {
                        ...
                        if (isAtEndpointA || isAtEndpointB)
                            continue;
            ...
            if (wireStretchState.dragging) {
                ...
                if (wireStretchState.kind === 'free' && !isHorizontal && !isVertical) {
                    ...
                    if (wireStretchState.slidingRubberBands.length > 0) {
                        ...
                        for (const rb of wireStretchState.slidingRubberBands) {
                            if (endpointAttachWhich.has(rb.which))
                                continue;
                            const host = wires.find(ww => ww.id === rb.hostWireId);
                            if (!host)
                                continue;
                            if (rb.hostSegIndex < 0 || rb.hostSegIndex >= host.points.length - 1)
                                continue;
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter' && (mode === 'place-junction' || mode === 'delete-junction')) {
            setMode('select');
        }
            ...
            if (seg) {
                ...
                if (isNonOrtho) {
                    ...
                    for (const w of wires) {
                        ...
                        if (key0 !== startKey && key1 !== startKey)
                            continue;
    ...
    function updatePolarInputs(x, y) {
        if (!coordInputLength || !coordInputAngle || !polarInputGroup)
            return;
        // Don't update if user is actively typing
        if (coordInputActive)
            return;
    ...
    function acceptPolarInput() {
        if (!coordInputLength || !coordInputAngle || !drawing.active || drawing.points.length === 0)
            return null;
    ...
    if (coordInputLength && coordInputAngle) {
        ...
        coordInputLength.addEventListener('blur', () => {
            setTimeout(() => {
                if (document.activeElement !== coordInputAngle)
                    coordInputActive = false;
            }, 100);
        });
        coordInputAngle.addEventListener('blur', () => {
            setTimeout(() => {
                if (document.activeElement !== coordInputLength)
                    coordInputActive = false;
            }, 100);
        });
    }
    ...
    function handleCoordinateInputClick(point) {
        ...
        if (mode === 'move' && coordSel && coordSel.kind === 'component') {
            ...
            components.push(comp);
            breakWiresForComponent(comp);
            if (shouldTreatAsTwoPin(placeType))
                deleteBridgeBetweenPins(comp);
            setMode('select');
            placeType = null;
            clearLibraryPlacementState();
    ...
    function updateSubtypeVisibility() {
        ...
        if (show) {
            ...
            const capacitorSubtypes = document.querySelector('.capacitor-subtypes');
            const transistorSubtypes = document.querySelector('.transistor-subtypes');
            if (ds)
                ds.style.display = (placeType === 'diode') ? 'inline-block' : 'none';
            if (capacitorSubtypes)
                capacitorSubtypes.style.display = (placeType === 'capacitor') ? 'flex' : 'none';
            if (transistorSubtypes)
                transistorSubtypes.style.display = (placeType === 'npn' || placeType === 'pnp') ? 'flex' : 'none';
            if (ds && placeType === 'diode')
                ds.value = diodeSubtype;
            if (capacitorSubtypes && placeType === 'capacitor')
                updateCapacitorSubtypeButtons();
            if (transistorSubtypes && (placeType === 'npn' || placeType === 'pnp'))
    ...
    function updateTransistorSubtypeButtons() {
        ...
        if (npnBtn)
            npnBtn.classList.toggle('active', transistorType === 'npn');
        if (pnpBtn)
            pnpBtn.classList.toggle('active', transistorType === 'pnp');
    }
    ...
    (function installUnitsSelect() {
        ...
        if (junctionCustomSizeInput) {
            ...
            updateJunctionSizeUI();
            let lastValidSizeMils = junctionCustomSize; // Track last valid size for nearest preset calculation
            const handleCustomSizeChange = () => {
                ...
                if (parsed && parsed.nm > 0) {
                    ...
                    if (lastValidSizeMils !== null) {
                        ...
                        const presetNames = ['smallest', 'small', 'default', 'large', 'largest'];
        ...
        if (junctionDefaultColorPicker && junctionDefaultOpacity) {
            ...
            // Add color swatches
            const junctionColorSwatches = $q('#junctionColorSwatches');
    ...
    function renderInspector() {
        Inspector.renderInspector({
            ...
            removeLibrary: (libraryName) => removeSymbolLibrary(libraryName),
            onLibrarySymbolSelect: (libraryName, symbolName) => handleLibrarySymbolSelect(libraryName, symbolName),
    ...
    function moveComponentWithConstraints(dx, dy) {
        ...
        if (temporarilyDisabled.length > 0) {
            for (const cons of temporarilyDisabled)
                cons.enabled = true;
        }
        if (result.allowed) {
            // Move the component
    ...
    function handleKicadSymbolImport(file) {
        const reader = new FileReader();
        reader.onload = () => {
            ...
            const nextIndex = { ...projectSettings.symbolLibraries, [libraryKey]: deduped };
            projectSettings = updateProjectSettings({ symbolLibraries: nextIndex });
    ...
    function deriveLibraryKey(fileName) {
    ...
    function extractTopLevelKiCadSymbols(content) {
        const names = [];
        let depth = 0;
    ...
    function parsePinVisibilityFlag(symbolBlock, key) {
        ...
        if (/\(hide\s+yes\)/i.test(section))
            return false;
        if (/\(hide\s+no\)/i.test(section))
            return true;
        if (/\(show\s+yes\)/i.test(section))
            return true;
        if (/\(show\s+no\)/i.test(section))
            return false;
        return null;
    }
    function parseKiCadProperties(symbolBlock) {
        const props = {};
        let searchIndex = 0;
        while (true) {
            ...
            if (justifyMatch) {
                ...
                if (tokens.includes('left'))
                    entry.anchor = 'start';
                else if (tokens.includes('right'))
                    entry.anchor = 'end';
                else if (tokens.includes('center'))
                    entry.anchor = 'middle';
            }
            props[name] = entry;
        }
    ...
    function parseLibrarySymbol(libraryName, symbolName, visiting = new Set()) {
        ...
        try {
            ...
            let inheritedPinNumbers = null;
            let inheritedPinNames = null;
            let extendsMatch;
    ...
    function extractSymbolBlock(content, symbolName) {
    ...
    const mmToPxExact = (mm) => mm * PX_PER_MM;
    function parseKiCadPins(symbolBlock) {
        const pins = [];
        let searchIndex = 0;
        while (true) {
            ...
            const validTypes = ['input', 'output', 'bidirectional', 'power_in', 'power_out', 'passive', 'unspecified'];
            const normalizedType = validTypes.includes(rawType) ? rawType : 'unspecified';
            const pin = {
                id: numberMatch[1] || '0',
                x: mmToPxExact(xMm),
                y: -mmToPxExact(yMm),
    ...
    function parseKiCadGraphics(symbolBlock) {
        const graphics = [];
    ...
    function extractNestedSymbolBodies(symbolBlock) {
        const bodies = [];
    ...
    function parseSymbolBodyGraphics(bodyBlock) {
        const graphics = [];
        // Polyline primitives
        let searchIndex = 0;
        while (true) {
    ...
    function rebuildTopology() {
        ...
        function segsCross(s1, s2) {
            ...
            if (s1.a.x === s1.b.x && s2.a.y === s2.b.y) {
                // s1 vertical, s2 horizontal
                if (Math.min(s1.a.y, s1.b.y) < s2.a.y && s2.a.y < Math.max(s1.a.y, s1.b.y) &&
                ...
                // s1 horizontal, s2 vertical
                if (Math.min(s2.a.y, s2.b.y) < s1.a.y && s1.a.y < Math.max(s2.a.y, s2.b.y) &&
        ...
        if (isKicadMode && intersectionPoints.length) {
            ...
            for (const k of intersectionPoints) {
                if (manual.has(k) || suppressed.has(k))
                    continue;
        ...
        for (const w of wires) {
            ...
            for (let i = 1; i < pts.length; i++) {
                ...
                for (const k of intersectionPoints) {
                    const [ix, iy] = k.split(',').map(Number);
                    // Is this intersection on this segment?
                    if (((a.x === b.x && a.x === ix && Math.min(a.y, b.y) < iy && iy < Math.max(a.y, b.y)) ||
        ...
        for (const c of components) {
            ...
            if (wiresAtA.length !== 1 || wiresAtB.length !== 1)
                continue;
            // Also ensure they're different wires
            if (wiresAtA[0].id === wiresAtB[0].id)
                continue;
    ...
    function ensureFinishSwpMove() {
        ...
        if (!comp && moveCollapseCtx && moveCollapseCtx.sid) {
            const sid = moveCollapseCtx.sid;
            // Try direct mapping
            const compId = (() => {
            ...
            if (compId)
                comp = components.find(x => x.id === compId);
    ...
    (function initPanels() {
        ...
        try {
            ...
            if (saved) {
                ...
                if (parsed.left)
                    panelState.left = parsed.left;
                if (parsed.right)
                    panelState.right = parsed.right;
            }
        }
        catch (_) { }
        function saveState() {
        ...
        if (leftPanel) {
            const leftHeader = leftPanel.querySelector('.panel-header h2');
            if (panelState.left.collapsed) {
                leftPanel.classList.add('collapsed');
                leftPanel.style.width = getCollapsedWidth() + 'px';
                if (leftHeader)
                    leftHeader.textContent = 'I';
            }
            else {
                leftPanel.style.width = panelState.left.width + 'px';
                if (leftHeader)
                    leftHeader.textContent = 'Inspector';
            }
        }
        if (rightPanel) {
            const rightHeader = rightPanel.querySelector('.panel-header h2');
            if (panelState.right.collapsed) {
                rightPanel.classList.add('collapsed');
                rightPanel.style.width = getCollapsedWidth() + 'px';
                if (rightHeader)
                    rightHeader.textContent = 'P';
            }
            else {
                rightPanel.style.width = panelState.right.width + 'px';
                if (rightHeader)
                    rightHeader.textContent = 'Project';
            }
        }
        // Update button indicators based on state
        const leftToggle = document.querySelector('[data-panel="left"]');
        const rightToggle = document.querySelector('[data-panel="right"]');
        if (leftToggle)
            leftToggle.textContent = panelState.left.collapsed ? '▶' : '◀';
        if (rightToggle)
            rightToggle.textContent = panelState.right.collapsed ? '◀' : '▶';
