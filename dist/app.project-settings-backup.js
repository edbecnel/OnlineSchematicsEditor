...
(function () {
    ...
    originalP0: Point;
    originalP1: Point;
    connectedWiresStart: Array;
    connectedWiresEnd: Array;
    componentsOnWire: Array;
    ...
    ghostConnectingWires: Array; // Visual feedback for connecting wires
...
(function attachThemeToggle() {
    ...
    function applyTheme(theme) {
        ...
        applyProjectThemeBackground();
        redraw();
        renderDrawing();
    }
    function toggleTheme() {
        const newTheme = currentTheme === 'dark' ? 'light' : 'dark';
        suppressAutoTheme = true;
        applyTheme(newTheme);
        suppressAutoTheme = false;
    }
    setThemeFn = (mode) => {
    ...
    getThemeFn = () => currentTheme;
    applyTheme(currentTheme);
    autoAdjustThemeForBackground();
    if (themeBtn) {
...
function syncConstraints() {
    ...
    for (const c of components) {
        ...
        for (let i = components.indexOf(c) + 1; i < components.length; i++) {
            ...
            const clearanceOverridePx = projectSettings.componentClearancePx > 0
                ? projectSettings.componentClearancePx
                : undefined;
            constraintSolver.addConstraint({
...
function loadBackgroundCustomColors() {
    backgroundCustomColors = [];
    try {
        const raw = localStorage.getItem(BACKGROUND_CUSTOM_STORAGE_KEY);
        if (!raw)
            return;
        const parsed = JSON.parse(raw);
        ...
        for (const value of parsed) {
            ...
            if (seen.has(key))
                continue;
            seen.add(key);
            backgroundCustomColors.push(hex);
        }
    }
    catch {
        backgroundCustomColors = [];
...
function saveBackgroundCustomColors() {
    try {
        localStorage.setItem(BACKGROUND_CUSTOM_STORAGE_KEY, JSON.stringify(backgroundCustomColors));
    }
    catch {
        /* ignore */
    }
...
function createBackgroundSwatch(color, permanent) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'color-swatch';
    btn.dataset.color = color;
    btn.dataset.permanent = permanent ? 'true' : 'false';
    btn.style.width = '32px';
    btn.style.height = '32px';
    btn.style.borderRadius = '4px';
    btn.style.border = permanent ? '2px solid var(--muted)' : '2px solid transparent';
    btn.style.background = color;
    btn.style.cursor = 'pointer';
    btn.title = color;
    btn.addEventListener('click', () => selectBackgroundColor(color));
    return btn;
}
function markBackgroundSelection(color) {
    if (!backgroundSwatchContainer)
        return;
    const swatches = Array.from(backgroundSwatchContainer.querySelectorAll('.color-swatch'));
    let selectedSwatch = null;
    swatches.forEach(swatch => {
        const isMatch = colorsEqual(swatch.dataset.color, color);
        swatch.classList.toggle('selected', isMatch);
        swatch.style.borderColor = isMatch ? 'var(--accent)' : (swatch.dataset.permanent === 'true' ? 'var(--muted)' : 'transparent');
        if (isMatch)
            selectedSwatch = swatch;
    });
    if (deleteBackgroundColorBtn) {
        const isPermanent = selectedSwatch?.dataset.permanent === 'true';
        deleteBackgroundColorBtn.style.display = isPermanent ? 'none' : 'inline-flex';
    }
}
function rebuildBackgroundSwatches() {
    if (!backgroundSwatchContainer)
        return;
    backgroundSwatchContainer.innerHTML = '';
    const merged = [...BACKGROUND_SWATCHES.map(normalizeHexColor), ...backgroundCustomColors.map(normalizeHexColor)];
    const seen = new Set();
    merged.forEach((color, index) => {
        ...
        if (seen.has(key))
            return;
        seen.add(key);
        const swatch = createBackgroundSwatch(color, index < BACKGROUND_SWATCHES.length);
        backgroundSwatchContainer.appendChild(swatch);
    });
    markBackgroundSelection(projectSettings.theme.background);
}
function ensureBackgroundSwatchExists(color) {
    const normalized = normalizeHexColor(color);
    if (backgroundCustomColors.some(c => colorsEqual(c, normalized)))
        return;
    backgroundCustomColors.push(normalized);
    saveBackgroundCustomColors();
    rebuildBackgroundSwatches();
}
function deleteSelectedBackgroundColor() {
    if (!backgroundColorInput)
        return;
    const current = backgroundColorInput.value;
    backgroundCustomColors = backgroundCustomColors.filter(c => !colorsEqual(c, current));
    saveBackgroundCustomColors();
    rebuildBackgroundSwatches();
    selectBackgroundColor(BACKGROUND_SWATCHES[0] ?? '#101418');
}
function applyProjectThemeBackground() {
    const hex = normalizeHexColor(projectSettings.theme.background);
    document.documentElement.style.setProperty('--bg', hex);
    document.documentElement.style.setProperty('--light-bg', hex);
    document.body.style.background = hex;
    markBackgroundSelection(hex);
    if (backgroundColorInput) {
        backgroundColorInput.value = hex;
        backgroundColorInput.title = hex;
    }
}
function backgroundLuminance(color) {
    const rgba = cssToRGBA01(color);
    const toLinear = (v) => {
        if (v <= 0.03928)
            return v / 12.92;
...
function autoAdjustThemeForBackground() {
    if (suppressAutoTheme)
        return;
    const desired = backgroundLuminance(projectSettings.theme.background) > 0.6 ? 'light' : 'dark';
    if (getThemeFn() !== desired) {
        suppressAutoTheme = true;
        setThemeFn(desired);
        suppressAutoTheme = false;
    }
...
function selectBackgroundColor(color) {
    ...
    projectSettings = updateProjectSettings({ theme: { background: hex } });
    applyProjectThemeBackground();
    if (!suppressAutoTheme) {
        autoAdjustThemeForBackground();
    }
}
function refreshSymbolThemeInputs() {
    const theme = projectSettings.theme.symbol;
    ['body', 'pin', 'pinText', 'referenceText', 'valueText', 'powerSymbol'].forEach(key => {
        const input = symbolColorInputs[key];
        if (!input)
            return;
        const rgba = theme[key];
        const css = rgba01ToCss(rgba);
        const hex = colorToHex(css);
        input.value = hex;
        input.title = hex;
    });
}
function refreshProjectSettingsUI() {
    applyProjectThemeBackground();
    refreshSymbolThemeInputs();
    if (updateComponentClearanceUI)
        updateComponentClearanceUI();
}
function createSymbolColorControl(label, key) {
    ...
    symbolColorInputs[key] = input;
    return row;
}
function installProjectSettingsUI(settingsContent) {
    if (settingsContent.querySelector('#projectSettingsBlock'))
        return;
    loadBackgroundCustomColors();
    const block = document.createElement('div');
    block.id = 'projectSettingsBlock';
    ...
    // Component clearance
    const clearanceLabelRow = document.createElement('div');
    clearanceLabelRow.className = 'row';
    const clearanceLabel = document.createElement('label');
    clearanceLabel.textContent = 'Component Clearance';
    clearanceLabelRow.appendChild(clearanceLabel);
    block.appendChild(clearanceLabelRow);
    const clearanceInputRow = document.createElement('div');
    clearanceInputRow.className = 'row';
    const clearanceInput = dimNumberPx(projectSettings.componentClearancePx, (px) => {
        const clamped = Math.max(0, Math.round(px));
        projectSettings = updateProjectSettings({ componentClearancePx: clamped });
        if (USE_CONSTRAINTS && constraintSolver) {
            syncConstraints();
        }
        if (updateComponentClearanceUI)
    ...
    clearanceInput.id = 'componentClearanceInput';
    clearanceInput.title = 'Minimum spacing enforced between component bodies. Pins may still touch.';
    clearanceInputRow.appendChild(clearanceInput);
    block.appendChild(clearanceInputRow);
    updateComponentClearanceUI = () => {
        const currPx = projectSettings.componentClearancePx;
        clearanceInput.value = formatDimForDisplay(pxToNm(currPx), globalUnits);
    };
    // Background controls
    const backgroundLabelRow = document.createElement('div');
    backgroundLabelRow.className = 'row';
    const backgroundLabel = document.createElement('label');
    backgroundLabel.textContent = 'Background';
    backgroundLabelRow.appendChild(backgroundLabel);
    block.appendChild(backgroundLabelRow);
    const backgroundRow = document.createElement('div');
    backgroundRow.className = 'row';
    backgroundRow.style.display = 'flex';
    backgroundRow.style.flexWrap = 'wrap';
    backgroundRow.style.gap = '0.5rem';
    backgroundRow.style.alignItems = 'flex-end';
    backgroundSwatchContainer = document.createElement('div');
    backgroundSwatchContainer.id = 'backgroundColorSwatches';
    backgroundSwatchContainer.style.display = 'flex';
    backgroundSwatchContainer.style.flexWrap = 'wrap';
    backgroundSwatchContainer.style.gap = '0.25rem';
    backgroundSwatchContainer.style.marginBottom = '0.5rem';
    backgroundSwatchContainer.style.flex = '1 1 220px';
    const backgroundControlsWrap = document.createElement('div');
    backgroundControlsWrap.style.display = 'flex';
    backgroundControlsWrap.style.gap = '0.5rem';
    backgroundControlsWrap.style.alignItems = 'center';
    backgroundColorInput = document.createElement('input');
    backgroundColorInput.type = 'color';
    backgroundColorInput.id = 'backgroundColorPicker';
    backgroundColorInput.style.flex = '1';
    backgroundColorInput.style.height = '32px';
    backgroundColorInput.style.cursor = 'pointer';
    backgroundColorInput.addEventListener('input', () => {
        if (!backgroundColorInput)
            return;
        selectBackgroundColor(backgroundColorInput.value);
    });
    backgroundColorInput.addEventListener('change', () => {
        if (!backgroundColorInput)
            return;
        ensureBackgroundSwatchExists(backgroundColorInput.value);
        selectBackgroundColor(backgroundColorInput.value);
    });
    deleteBackgroundColorBtn = document.createElement('button');
    deleteBackgroundColorBtn.type = 'button';
    deleteBackgroundColorBtn.id = 'deleteBackgroundColorBtn';
    deleteBackgroundColorBtn.textContent = '✕';
    deleteBackgroundColorBtn.title = 'Delete selected custom color';
    deleteBackgroundColorBtn.style.width = '32px';
    deleteBackgroundColorBtn.style.height = '32px';
    deleteBackgroundColorBtn.style.background = 'var(--danger)';
    deleteBackgroundColorBtn.style.color = '#ffffff';
    deleteBackgroundColorBtn.style.border = 'none';
    deleteBackgroundColorBtn.style.borderRadius = '4px';
    deleteBackgroundColorBtn.style.cursor = 'pointer';
    deleteBackgroundColorBtn.style.display = 'none';
    deleteBackgroundColorBtn.style.alignItems = 'center';
    deleteBackgroundColorBtn.style.justifyContent = 'center';
    deleteBackgroundColorBtn.addEventListener('click', () => deleteSelectedBackgroundColor());
    backgroundControlsWrap.appendChild(backgroundColorInput);
    backgroundControlsWrap.appendChild(deleteBackgroundColorBtn);
    backgroundRow.appendChild(backgroundSwatchContainer);
    backgroundRow.appendChild(backgroundControlsWrap);
    block.appendChild(backgroundRow);
    const backgroundHint = document.createElement('div');
    backgroundHint.className = 'row hint';
    backgroundHint.textContent = 'Applied to both dark and light themes; accents flip automatically for contrast.';
    block.appendChild(backgroundHint);
    // Symbol theme controls
    const themeHeader = document.createElement('div');
    themeHeader.className = 'row';
    const themeHeaderLabel = document.createElement('label');
    themeHeaderLabel.textContent = 'Symbol Theme';
    themeHeaderLabel.style.width = 'auto';
    themeHeaderLabel.style.fontWeight = '600';
    themeHeader.appendChild(themeHeaderLabel);
    block.appendChild(themeHeader);
    block.appendChild(createSymbolColorControl('Body', 'body'));
    block.appendChild(createSymbolColorControl('Pins', 'pin'));
    block.appendChild(createSymbolColorControl('Pin Text', 'pinText'));
    block.appendChild(createSymbolColorControl('Reference Text', 'referenceText'));
    block.appendChild(createSymbolColorControl('Value Text', 'valueText'));
    block.appendChild(createSymbolColorControl('Power Symbols', 'powerSymbol'));
    rebuildBackgroundSwatches();
...
(function installUnitsSelect() {
    ...
    const settingsSection = document.querySelector('[data-section-content="settings"]');
    if (settingsSection) {
        installProjectSettingsUI(settingsSection);
    }
...
();
//# sourceMappingURL=app.project-settings-backup.js.map