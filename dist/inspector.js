...
let libraryCollapseState = loadLibraryCollapseState();
let libraryContextMenuEl = null;
let libraryContextMenuRemoveBtn = null;
let libraryContextMenuVisible = false;
let libraryContextMenuTarget = null;
let libraryContextMenuRemoveFn = null;
...
function loadLibraryCollapseState() {
...
function isLibraryCollapsed(libraryName) {
    return !!libraryCollapseState[libraryName];
}
function setLibraryCollapsed(libraryName, collapsed) {
    if (!libraryName)
        return;
    if (collapsed) {
...
function ensureLibraryContextMenu() {
    ...
    if (!libraryContextMenuHandlersAttached) {
        document.addEventListener('click', (event) => {
            if (!libraryContextMenuVisible)
                return;
            if (libraryContextMenuEl && libraryContextMenuEl.contains(event.target))
                return;
            hideLibraryContextMenu();
        });
...
function showLibraryContextMenu(x, y, libraryName, removeFn) {
...
export function renderInspector(ctx, inspector, inspectorNone) {
    ...
    if (ctx.inspectorTab === 'libraries') {
        ...
        if (Object.keys(ctx.symbolLibraries).length === 0) {
            ...
            const sectionsData = [];
            let noResults = null;
            function applyCollapseStateToSection(data, collapsed) {
...
export function clearLibraryCollapseState(libraryName) {
    if (!libraryName)
        return;
