...
export function createDefaultValidators() {
    ...
    validators.set('min-distance', {
        validate: (entity, proposedPosition, affectedEntities, context, constraint) => {
            ...
            const paramsAll = constraint.params;
            // If scoped to a specific pair and this move matches, allow relaxation
            const between = paramsAll.betweenIds;
    ...
    validators.set('pin-touch', {
        validate: (entity, proposedPosition, affectedEntities, context, constraint) => {
            ...
            const gridOnly = !!params.gridOnly;
            const pinIndex = typeof params.pinIndex === 'number' ? params.pinIndex : 0;
            const targetId = params.targetEntityId;
            const targetPinIndex = typeof params.targetPinIndex === 'number' ? params.targetPinIndex : undefined;
            ...
            const pinOffsets = entity.metadata?.pinOffsets;
            ...
            const targetPinOffsets = target.metadata?.pinOffsets;
