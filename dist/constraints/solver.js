...
export class ConstraintSolver {
    constructor(snapToGrid, snapToBaseScalar) {
        this.snapToGrid = snapToGrid;
        this.snapToBaseScalar = snapToBaseScalar;
     ...
     */
    solve(entityId, proposedPosition) {
        ...
        const context = {
            allEntities: new Map(this.graph.getAllEntities().map(e => [e.id, e])),
            allConstraints: new Map(this.graph.getAllConstraints().map(c => [c.id, c])),
     ...
     */
    explainViolations(entityId, proposedPosition) {
        const result = this.solve(entityId, proposedPosition);
        if (result.allowed) {
            return ['Move is allowed'];
        }
        return result.violatedConstraints.map(v => `[${v.constraint.type}] ${v.reason} (priority: ${v.constraint.priority})`);
    }
    /**
