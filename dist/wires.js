...
import { dist2, keyPt, projectPointToSegmentWithT, nearestPointOnSegment } from './geometry.js';
// ========================================================================================
 ...
 */
export function breakWiresAtPins(pins, wires, uid, snapToBaseScalar, GRID) {
    let broke = false;
 ...
 */
export function breakNearestWireAtPin(pin, wires, uid, snapToBaseScalar, GRID) {
 ...
 */
export function mendWireAtPoints(hitA, hitB, wires, uid, defaultWireColor) {
    if (!hitA || !hitB)
        return wires;
    ...
    if (merged.length >= 2) {
        ...
        const rgba01ToCss = (c) => `rgba(${Math.round(c.r * 255)},${Math.round(c.g * 255)},${Math.round(c.b * 255)},${c.a})`;
 ...
 */
export function findWireEndpointNear(pt, wires, tol = 0.9) {
    for (const w of wires) {
 ...
 */
export function deleteBridgeBetweenPins(pins, wires) {
    if (pins.length !== 2)
        return wires;
 ...
 */
export function normalizeAllWires(wires, uid, defaultWireColor) {
    const next = [];
    for (const w of wires) {
 ...
 */
export function splitPolylineByRemovedSegments(pts, removeIdxSet) {
    if (!pts || pts.length < 2)
        return [];
    const out = [];
    let cur = [pts[0]];
 ...
 * Returns array of normalized point arrays (each ≥ 2 points).
 */
export function splitPolylineByKeptSegments(pts, keepIdxSet) {
    if (!pts || pts.length < 2)
        return [];
    const out = [];
    let cur = [];
 ...
 */
export function isolateWireSegment(w, segIndex, wires, uid) {
    if (!w)
 ...
 */
function allPinKeys(components, compPinPositions) {
    const s = new Set();
 ...
 */
export function unifyInlineWires(wires, components, compPinPositions, snapToBaseScalar, uid, defaultWireColor) {
 ...
 */
export function wiresEndingAt(pt, wires) {
    const eqPt = (a, b) => Math.abs(a.x - b.x) < 1e-3 && Math.abs(a.y - b.y) < 1e-3;
    return wires.filter(w => {
 ...
 */
export function otherEndpointOf(w, endPt) {
    const eqPt = (a, b) => Math.abs(a.x - b.x) < 1e-3 && Math.abs(a.y - b.y) < 1e-3;
    const a = w.points[0], b = w.points[w.points.length - 1];
 ...
 */
export function adjacentOther(w, endPt) {
    const eqPt = (a, b) => Math.abs(a.x - b.x) < 1e-3 && Math.abs(a.y - b.y) < 1e-3;
    const n = w.points.length;
    if (n < 2)
        return null;
