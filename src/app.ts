...
(function () {
  ...
  function temporarilyAllowOnGridConnectionsForComponent(comp: Component) {
    ...
    try {
      ...
      for (const con of constraintsForComp) {
        if (con.type === 'min-distance' && con.params && (con.params as any).betweenIds) {
          // Remove the betweenIds marker so normal min-distance applies by default
          (con.params as any).betweenIds = undefined;
        }
      ...
      for (const other of allComps) {
        ...
        if (shouldRelaxPair) {
          ...
          if (pairConstraint && pairConstraint.params) {
            (pairConstraint.params as any).betweenIds = [comp.id, other.id];
            if (pairConstraint.metadata) pairConstraint.metadata.temporary = true;
  ...
  let wireStretchState: { 
    kind: 'orthogonal' | 'free';
    wire: Wire; 
    startMousePos: Point; 
    originalPoints: Point[];
    originalP0: Point;
    originalP1: Point;
    // For free-angle moves: if an endpoint was T-junctioned (endpoint-on-segment) into another wire,
    // render a rubber-band segment whose junction point slides along the host wire.
    stubbedInto: Array<{
      which: 'start' | 'end';
      hostWireId: string;
      hostSegIndex: number;
      ...
      hostEndIndex?: number; // only for attach==='endpoint'
    }>;
    // For subsequent drags: if there is already a rubber-band wire connected to this moved wire,
    // and the far end of that rubber-band is stubbed into a host wire segment, keep that far end
    // sliding along the host segment while dragging.
    slidingRubberBands: Array<{
      which: 'start' | 'end';
      wire: Wire;
      movingEndIndex: number;
      hostEndIndex: number;
      hostWireId: string;
      hostSegIndex: number;
      hostT: number;
  ...
  function pinOffsetsFor(c: Component): Point[] {
    const pinsNow = Components.compPinPositions(c);
    return pinsNow.map(p => ({ x: p.x - c.x, y: p.y - c.y }));
  }
  function pinsAt(c: Component, x: number, y: number, cachedOffsets?: Point[]): Point[] {
    const offs = cachedOffsets || pinOffsetsFor(c);
    return offs.map(o => ({ x: x + o.x, y: y + o.y }));
    }
  function intervalAlongAxis(pts: Point[], axis: 'x' | 'y'): { min: number, max: number } {
    if (axis === 'x') {
      ...
      let mn = Infinity, mx = -Infinity;
      for (const p of pts) { if (p.y < mn) mn = p.y; if (p.y > mx) mx = p.y; }
      return { min: mn, max: mx };
    }
  }
  function intervalsTrulyOverlap(a: { min: number, max: number }, b: { min: number, max: number }): boolean {
  ...
  function isAlignedToFixed(pts: Point[], axis: 'x' | 'y', fixed: number): boolean {
    if (axis === 'x') return pts.every(p => Math.abs(p.y - fixed) <= AXIS_EPS);
    else return pts.every(p => Math.abs(p.x - fixed) <= AXIS_EPS);
  }
  function wouldOverlapOnAxis(c: Component, candX: number, candY: number, axis: 'x' | 'y', fixed: number): boolean {
    const candPins = pinsAt(c, candX, candY);
    if (!isAlignedToFixed(candPins, axis, fixed)) return false; // not on that axis; off-axis check handles elsewhere
    const candInt = intervalAlongAxis(candPins, axis);
    for (const other of components) {
      if (other.id === c.id) continue;
      const otherPins = Components.compPinPositions(other);
      // Must be same axis and same fixed within tolerance
      if (!isAlignedToFixed(otherPins, axis, fixed)) continue;
      const otherInt = intervalAlongAxis(otherPins, axis);
      if (intervalsTrulyOverlap(candInt, otherInt)) return true; // only block true penetration; touching is ok
    }
    return false;
  }

  // ---- Bridging helpers for mending gaps when separating components on a wire ----
  function eqCoord(a: number, b: number, eps = 1e-3) { return Math.abs(a - b) <= eps; }
  function eqPoint(a: Point, b: Point, eps = 1e-3) { return Math.hypot(a.x - b.x, a.y - b.y) <= eps; }
  function segmentExistsBetween(a: Point, b: Point): boolean {
    for (const w of wires) {
      if (w.points.length !== 2) continue;
      const p0 = w.points[0], p1 = w.points[1];
      if ((eqPoint(p0, a) && eqPoint(p1, b)) || (eqPoint(p0, b) && eqPoint(p1, a))) return true;
    }
    return false;
  }
  function collectCollinearNeighbors(axis: 'x' | 'y', fixed: number, excludeCompId?: string): Point[] {
    const pts: Point[] = [];
    // Wire endpoints
    for (const w of wires) {
      if (w.points.length !== 2) continue;
      ...
      if (axis === 'x') {
        if (eqCoord(a.y, fixed)) pts.push({ x: a.x, y: a.y });
        if (eqCoord(b.y, fixed)) pts.push({ x: b.x, y: b.y });
      } else {
        if (eqCoord(a.x, fixed)) pts.push({ x: a.x, y: a.y });
        if (eqCoord(b.x, fixed)) pts.push({ x: b.x, y: b.y });
      }
    }
    // Junction dots
    for (const j of junctions) {
      if (axis === 'x') { if (eqCoord(j.at.y, fixed)) pts.push({ x: j.at.x, y: j.at.y }); }
      else { if (eqCoord(j.at.x, fixed)) pts.push({ x: j.at.x, y: j.at.y }); }
    }
    ...
    for (const comp of components) {
      ...
      const pins = Components.compPinPositions(comp);
      for (const p of pins) {
        if (axis === 'x') { if (eqCoord(p.y, fixed)) pts.push({ x: snapToBaseScalar(p.x), y: snapToBaseScalar(p.y) }); }
        else { if (eqCoord(p.x, fixed)) pts.push({ x: snapToBaseScalar(p.x), y: snapToBaseScalar(p.y) }); }
      }
    }
    return pts;
  }
  function nearestNeighborOnAxis(from: Point, axis: 'x' | 'y', fixed: number, dir: -1 | 1, excludeCompId?: string): Point | null {
    const pts = collectCollinearNeighbors(axis, fixed, excludeCompId);
    let best: Point | null = null;
    let bestD = Infinity;
    for (const p of pts) {
      if (eqPoint(p, from)) continue;
      const d = axis === 'x' ? (p.x - from.x) : (p.y - from.y);
      if (dir === -1 && d >= 0) continue;
      if (dir === 1 && d <= 0) continue;
      const ad = Math.abs(d);
      if (ad < bestD) { bestD = ad; best = p; }
    }
    return best;
  }
  // Check if a pin already has an outward collinear segment in the given direction
  function hasOutwardSegmentFromPin(pin: Point, axis: 'x' | 'y', dir: -1 | 1): boolean {
    const EPS = 1e-3;
    for (const w of wires) {
      if (w.points.length !== 2) continue;
      ...
      if (eqPoint(a, pin, EPS)) other = b; else if (eqPoint(b, pin, EPS)) other = a;
      if (!other) continue;
      // Must be collinear on the same fixed axis
      if (axis === 'x') {
        if (Math.abs(other.y - pin.y) > EPS) continue;
        const d = other.x - pin.x;
        if ((dir === 1 && d > EPS) || (dir === -1 && d < -EPS)) return true;
      } else {
        if (Math.abs(other.x - pin.x) > EPS) continue;
    ...
    if (horiz || vert) {
      for (const w of wires) {
        ...
        if (horiz && Math.abs(a0.y - p0.y) < 1e-6 && Math.abs(b0.y - p0.y) < 1e-6) {
          ...
          if (sMin >= minX - 1e-6 && sMax <= maxX + 1e-6) return;
        }
        if (vert && Math.abs(a0.x - p0.x) < 1e-6 && Math.abs(b0.x - p0.x) < 1e-6) {
          ...
          if (sMin >= minY - 1e-6 && sMax <= maxY + 1e-6) return;
        }
      }
    }
    wires.push({ id: State.uid('wire'), points: [p0, p1], color: color || defaultWireColor, stroke: undefined } as Wire);
  }

  // Bridge only between facing pins of nearest neighbor components on the same axis
  function ensureFacingBridgesFor(comp: Component) {
    const pins = Components.compPinPositions(comp).map(p => ({ x: snapToBaseScalar(p.x), y: snapToBaseScalar(p.y) }));
    if (pins.length < 1) return;
    const axis: 'x' | 'y' = (pins.length >= 2 && Math.abs(pins[0].x - pins[1].x) < 0.1) ? 'y' : 'x';
    ...
    let rightNeighborPin: Point | null = null;
    let bestRightD = Infinity;
    let leftNeighborPin: Point | null = null;
    ...
    for (const other of components) {
      ...
      if (axis === 'x') {
        ...
        if (dR > 0 && dR < bestRightD) { bestRightD = dR; rightNeighborPin = oLeft; }
        const dL = leftPin.x - oRight.x; // neighbor to left
        if (dL > 0 && dL < bestLeftD) { bestLeftD = dL; leftNeighborPin = oRight; }
      } else {
  ...
  function ensureBridgingSegmentsForComponent(comp: Component) {
  ...
  (function attachModeButtons() {
    try {
      ...
      let selectCaptureActivated = false as boolean;
      let selectCapturePrevMode: EditorMode | null = null;
      ...
      for (const btn of buttons) {
        btn.addEventListener('click', (e) => {
          ...
          if (nextMode === 'select') {
            ...
            selectCaptureActivated = false; selectCapturePrevMode = null;
          } else {
            setMode(nextMode);
          }
          e.stopPropagation();
      ...
      const selectBtnEl = document.getElementById('selectModeBtn') as HTMLButtonElement | null;
      if (selectBtnEl) {
  ...
  (function attachOrthoToggle() {
    ...
    function toggleOrtho() {
      orthoMode = !orthoMode;
      saveOrthoMode();
      // Keep routing kernel preview mode in sync with Ortho toggle.
      // Legacy ignores this; KiCad kernel uses it to switch between manhattan vs straight preview.
      routingFacade.setLineDrawingMode(orthoMode ? 'orthogonal' : 'free');
      updateOrthoButton();
    }
    if (orthoBtn) {
    ...
    g.addEventListener('pointerdown', (e) => {
      ...
      if (!embeddedFlag) {
        ...
        if (hitAfb && hitBfb) {
          ...
          wsA = [hitAfb.w];
          wsB = [hitBfb.w];
          hitAForFallback = hitAfb as any;
          hitBForFallback = hitBfb as any;
      ...
      if (embeddedFlag) {
        ...
        if (wA.id !== wB.id && wAEndAtA !== null && wBEndAtB !== null) {
          ...
          if (axis) {
            ...
            if (alignedWires.length > 0 && min !== Infinity && max !== -Infinity) {
              ...
              if (!c.connectedTo) c.connectedTo = [];
        ...
        // NEW: If not in SWP mode, check for simple wire connections at each pin
        // Set up wire stretching for components with connected wires
        if (!moveCollapseCtx && !componentDragState) {
          ...
          for (let pinIdx = 0; pinIdx < pins0.length; pinIdx++) {
            ...
            for (const wire of connectedWires) {
              // Determine which endpoint of the wire connects to this pin
              const isStart = Geometry.eqPtEps(wire.points[0], pin, 1);
              const isEnd = Geometry.eqPtEps(wire.points[wire.points.length - 1], pin, 1);
              ...
              const isHorizontal = Math.abs(p0.y - p1.y) < Math.abs(p0.x - p1.x);
              const wireAxis: 'x' | 'y' = isHorizontal ? 'x' : 'y';
              
              // Find the far endpoint (opposite from component)
              const farEndpoint = isStart ? wire.points[wire.points.length - 1] : wire.points[0];
              
              // Check if far end has junction or component connection
              const farWires = wiresEndingAt(farEndpoint);
              const shouldBreakAtFarEnd = farWires.length > 1 || components.some(comp => {
                if (comp.id === c.id) return false;
                const compPins = Components.compPinPositions(comp);
                return compPins.some(cp => Geometry.eqPtEps(cp, farEndpoint, 1));
              });
              
              // Track perpendicular wires at far end
              const perpendicularWires: typeof componentDragState.wires[number]['perpendicularWires'] = [];
              if (shouldBreakAtFarEnd) {
                // Find wires perpendicular to this one at the far endpoint
                for (const perpWire of farWires) {
                  if (perpWire.id === wire.id) continue;
                  
                  const perpP0 = perpWire.points[0];
                  const perpP1 = perpWire.points[perpWire.points.length - 1];
                  const perpIsHorizontal = Math.abs(perpP0.y - perpP1.y) < Math.abs(perpP0.x - perpP1.x);
                  const perpAxis: 'x' | 'y' = perpIsHorizontal ? 'x' : 'y';
                  
                  // Only track if perpendicular to main wire
                  if (perpAxis !== wireAxis) {
                    const perpEndIndex = Geometry.eqPtEps(perpP0, farEndpoint, 1) ? 0 : 
                                       (Geometry.eqPtEps(perpP1, farEndpoint, 1) ? 1 : -1);
                    if (perpEndIndex >= 0) {
                      perpendicularWires.push({
                        wire: perpWire,
                        endpointIndex: perpEndIndex,
                        originalPoints: perpWire.points.map(pt => ({ x: pt.x, y: pt.y }))
                      });
                    }
                  }
                }
              ...
              componentDragState.wires.push({
                wire,
                pinIndex: pinIdx,
                originalPoints: wire.points.map(pt => ({ x: pt.x, y: pt.y })),
                originalPinPosition: { x: pin.x, y: pin.y },
                axis: wireAxis,
                isStart,
                shouldBreakAtFarEnd,
      ...
      if (slideCtx) {
        if (slideCtx.axis === 'x') {
          ...
          if (USE_CONSTRAINTS && constraintSolver) {
            ...
            if (!result.allowed) { dbg('move:block', { id: c.id, reason: 'constraints' }); return; }
            ...
            dbg('move:block', { id: c.id, reason: 'legacy-overlap' }); return;
          }
        ...
        const axisForLock: 'x' | 'y' | null = (c.embeddedAxis as any) || (chainInfo ? chainInfo.axis : null);
        const fixedForLock: number | null = (c.embeddedFixed as any) ?? (chainInfo ? chainInfo.fixed : null);
        ...
        if (axisForLock && fixedForLock !== null && fixedForLock !== undefined) {
          ...
          if (axisForLock === 'x') candY = fixedForLock; else candX = fixedForLock;
        ...
        if (axisForLock && fixedForLock !== null && fixedForLock !== undefined) {
          if (wouldOverlapOnAxis(c, candX, candY, axisForLock as 'x' | 'y', fixedForLock as number)) {
        ...
        if (componentDragState && componentDragState.wires.length > 0) {
          ...
          for (const dragWire of componentDragState.wires) {
            const currentPin = currentPins[dragWire.pinIndex];
            const pinDelta = {
              x: currentPin.x - dragWire.originalPinPosition.x,
              y: currentPin.y - dragWire.originalPinPosition.y
            };
            
            // Determine movement direction relative to wire axis
      ...
      if (dragStart) {
        ...
        if (componentDragState && componentDragState.wires.length > 0) {
          ...
          for (const dragWire of componentDragState.wires) {
            ...
            const pinDelta = {
              x: finalPin.x - dragWire.originalPinPosition.x,
              y: finalPin.y - dragWire.originalPinPosition.y
            };
            
            // Determine if movement was primarily along axis (inline/stretch) or perpendicular (move)
            const alongAxis = dragWire.axis === 'x' ? pinDelta.x : pinDelta.y;
            const perpToAxis = dragWire.axis === 'x' ? pinDelta.y : pinDelta.x;
        ...
        if (slideCtx && dragStart.embedded) {
          const componentsToBreakFor: Component[] = [c];
          if ((slideCtx as any).otherComponents) {
            componentsToBreakFor.push(...(slideCtx as any).otherComponents);
          }
          for (const embComp of componentsToBreakFor) {
            ...
            if (removed) dbg('cleanup:through-body-removed', { id: embComp.id, removed });
          }
        ...
        if (!USE_CONSTRAINTS && overlapsAnyOther(c)) {
          ...
          if (!dragStart.embedded) {
            ...
            if (stillEmbedded || chainFinal || (c.embeddedInWireId)) {
              ...
              let anyBroke = false;
              for (const embComp of componentsToBreakFor) {
                if (breakWiresForComponent(embComp)) anyBroke = true;
              }
              ...
              if (c.embeddedAxis && c.embeddedFixed !== null && c.embeddedFixed !== undefined) {
                ...
                if (c.embeddedAxis === 'x') c.y = c.embeddedFixed as number; else c.x = c.embeddedFixed as number;
                updateComponentDOM(c);
        ...
        if (mode === 'delete') { 
          ...
          if (mode === 'select' || mode === 'move') {
            ...
            if (mode === 'move' && currentSel && currentSel.kind === 'wire' && currentSel.id === w.id) {
              ...
              // Determine whether this is an orthogonal wire (existing stretch behavior) or a free-angle wire.
              const isHorizontal = Math.abs(p0.y - p1.y) < 1;
              const isVertical = Math.abs(p0.x - p1.x) < 1;
              const stretchKind: 'orthogonal' | 'free' = (isHorizontal || isVertical) ? 'orthogonal' : 'free';

              // Helper: detect if a wire endpoint lies on the interior of another wire segment (T-junction).
              // Returns the host wire id + segment index.
              const findStubHost = (
                pt: Point,
                extraExcludeWireIds?: Set<string>,
                includeSegmentEndpoints = false
              ): { hostWireId: string; hostSegIndex: number; hostT: number; hostPt: Point } | null => {
                const EPS = 0.01;
                const tol = 1.0;
                let best: { hostWireId: string; hostSegIndex: number; d: number; hostT: number; hostPt: Point } | null = null;
                for (const ww of wires) {
                  if (ww.id === w.id) continue;
                  if (extraExcludeWireIds && extraExcludeWireIds.has(ww.id)) continue;
                  for (let si = 0; si < ww.points.length - 1; si++) {
                    const a = ww.points[si];
                    const b = ww.points[si + 1];
                    ...
                    if (isVert) {
                      ...
                      if (pt.y < minY - tol || pt.y > maxY + tol) continue;
                      const d = Math.abs(pt.x - a.x);
                      if (d > tol) continue;
                      // Exclude near segment endpoints unless explicitly requested.
                      if (!includeSegmentEndpoints) {
                        // (avoid treating endpoint-to-endpoint as a stub)
                        if (Math.hypot(pt.x - a.x, pt.y - a.y) <= tol) continue;
                        if (Math.hypot(pt.x - b.x, pt.y - b.y) <= tol) continue;
                      }
                      const { proj, t } = Geometry.projectPointToSegmentWithT(pt, a, b);
                      const hostT = Math.max(0, Math.min(1, t));
                      const hostPt = { x: proj.x, y: proj.y };
                      if (!best || d < best.d) best = { hostWireId: ww.id, hostSegIndex: si, d, hostT, hostPt };
                    } else if (isHoriz) {
                      const minX = Math.min(a.x, b.x);
                      const maxX = Math.max(a.x, b.x);
                      if (pt.x < minX - tol || pt.x > maxX + tol) continue;
                      const d = Math.abs(pt.y - a.y);
                      if (d > tol) continue;
                      if (!includeSegmentEndpoints) {
                        if (Math.hypot(pt.x - a.x, pt.y - a.y) <= tol) continue;
                        if (Math.hypot(pt.x - b.x, pt.y - b.y) <= tol) continue;
                      ...
                      // Diagonal host: use projection distance.
                      const { proj, t } = Geometry.projectPointToSegmentWithT(pt, a, b);
                      if (!includeSegmentEndpoints && (t <= 0 || t >= 1)) continue;
                ...
                return best ? { hostWireId: best.hostWireId, hostSegIndex: best.hostSegIndex, hostT: best.hostT, hostPt: best.hostPt } : null;
              };

              const stubbedInto: Array<{
                which: 'start' | 'end';
                hostWireId: string;
                hostSegIndex: number;
                hostT: number;
              ...
              if (stretchKind === 'free' || ((window as any).routingKernelMode === 'kicad')) {
                ...
                // If there is already an ortho extension (rubber-band) wire connected to the moved wire,
                // its endpoint can coincide with the moved endpoint. On subsequent drags we must NOT treat
                // that extension wire as the "host"; instead we keep constraining its far end to the true host.
                // Detect such extension wires and exclude them from host detection.
                const rubberBandExclude = new Set<string>();
                const markRubberBandAt = (pt: Point) => {
                  const tol = 1.0;
                  for (const ww of wires) {
                    if (ww.id === w.id) continue;
                    ...
                    const atA = (Math.hypot(a.x - pt.x, a.y - pt.y) <= tol);
                    const atB = (Math.hypot(b.x - pt.x, b.y - pt.y) <= tol);
                    if (!atA && !atB) continue;
                    const far = atA ? b : a;
                    const stub = findStubHost(far, new Set([ww.id]), true);
                    if (stub) rubberBandExclude.add(ww.id);
                  }
                };
                markRubberBandAt(ep0);
                markRubberBandAt(ep1);
                const s0 = findStubHost(ep0, rubberBandExclude, true);
                if (s0) {
                  const host = wires.find(ww => ww.id === s0.hostWireId);
                  const a = host ? host.points[s0.hostSegIndex] : null;
                  const b = host ? host.points[s0.hostSegIndex + 1] : null;
                  const tol = 1.0;
                  let attach: 'segment' | 'endpoint' = 'segment';
                  let hostEndIndex: number | undefined;
                  if (host && a && b) {
                    const da = Math.hypot(ep0.x - a.x, ep0.y - a.y);
                    const db = Math.hypot(ep0.x - b.x, ep0.y - b.y);
                    if (da <= tol || db <= tol) {
                      attach = 'endpoint';
                      hostEndIndex = (da <= db) ? s0.hostSegIndex : (s0.hostSegIndex + 1);
                    }
                  }
                  stubbedInto.push({ which: 'start', hostWireId: s0.hostWireId, hostSegIndex: s0.hostSegIndex, hostT: s0.hostT, hostPt: s0.hostPt, attach, hostEndIndex });
                }
                const s1 = findStubHost(ep1, rubberBandExclude, true);
                if (s1) {
                  const host = wires.find(ww => ww.id === s1.hostWireId);
                  const a = host ? host.points[s1.hostSegIndex] : null;
                  const b = host ? host.points[s1.hostSegIndex + 1] : null;
                  const tol = 1.0;
                  let attach: 'segment' | 'endpoint' = 'segment';
                  let hostEndIndex: number | undefined;
                  if (host && a && b) {
                    const da = Math.hypot(ep1.x - a.x, ep1.y - a.y);
                    const db = Math.hypot(ep1.x - b.x, ep1.y - b.y);
                    if (da <= tol || db <= tol) {
                      attach = 'endpoint';
                      hostEndIndex = (da <= db) ? s1.hostSegIndex : (s1.hostSegIndex + 1);
                    }
                  }
                  stubbedInto.push({ which: 'end', hostWireId: s1.hostWireId, hostSegIndex: s1.hostSegIndex, hostT: s1.hostT, hostPt: s1.hostPt, attach, hostEndIndex });
                }
              }
              ...
              // For free-angle moves: detect any already-existing rubber-band wires connected to this wire,
              // whose far end is stubbed into a host wire segment (endpoint-on-segment).
              const slidingRubberBands: Array<{
                which: 'start' | 'end';
                wire: Wire;
                movingEndIndex: number;
                hostEndIndex: number;
              ...
              if (stretchKind === 'free') {
                const considerConn = (which: 'start' | 'end', conn: { wire: Wire; isStart: boolean; originalPoint: Point }) => {
                  const rubber = conn.wire;
                  const movingEndIndex = conn.isStart ? 0 : (rubber.points.length - 1);
                  ...
                  slidingRubberBands.push({
                    which,
                    wire: rubber,
                    movingEndIndex,
                    hostEndIndex,
                    hostWireId: stub.hostWireId,
                    hostSegIndex: stub.hostSegIndex,
                    hostT: stub.hostT,
                    hostPt: stub.hostPt
                  });
                };
  ...
  function findAllComponentsOnAlignedWires(targetComp: Component): Component[] {
    ...
    if (pins.length !== 2) return [targetComp];
    ...
    const alignedWires = wires.filter(w => {
      if (w.points.length !== 2) return false;
      const p0 = w.points[0], p1 = w.points[1];
      if (axis === 'x') return p0.y === fixed && p1.y === fixed;
      else return p0.x === fixed && p1.x === fixed;
    });
    
    if (alignedWires.length === 0) return [targetComp];
    
    // Find all components whose pins are on these aligned wires
    const embeddedComps: Component[] = [];
    for (const comp of components) {
      const compPins = Components.compPinPositions(comp);
      if (compPins.length !== 2) continue;
      ...
      if (wsA.length === 1 && wsB.length === 1) {
        ...
        if (onAligned) embeddedComps.push(comp);
      }
    }
  ...
  function removeThroughBodySegmentsForComponent(c: Component) {
    const pins = Components.compPinPositions(c);
    ...
    const axis: 'x'|'y' = Math.abs(a.x - b.x) < 0.1 ? 'y' : 'x';
    ...
    const keep: Wire[] = [];
    for (const w of wires) {
      if (w.points.length !== 2) { keep.push(w); continue; }
      const p0 = { x: snapToBaseScalar(w.points[0].x), y: snapToBaseScalar(w.points[0].y) };
      ...
      if (axis === 'x' && Math.abs(p0.y - fixed) < EPS && Math.abs(p1.y - fixed) < EPS) {
        collinear = true; u0 = p0.x; u1 = p1.x;
      } else if (axis === 'y' && Math.abs(p0.x - fixed) < EPS && Math.abs(p1.x - fixed) < EPS) {
        collinear = true; u0 = p0.y; u1 = p1.y;
      }
      if (!collinear) { keep.push(w); continue; }
  ...
  function isEmbeddedChain(c: Component): { axis: 'x' | 'y'; fixed: number } | null {
    const pins = Components.compPinPositions(c).map(p => ({ x: snapToBaseScalar(p.x), y: snapToBaseScalar(p.y) }));
    if (pins.length < 2) return null;
    const axis: 'x' | 'y' = Math.abs(pins[0].x - pins[1].x) < 0.1 ? 'y' : 'x';
    const fixed = axis === 'x' ? pins[0].y : pins[0].x;

    const endHasWire = (pt: Point) => {
    ...
    const pinCoincidesWithOtherCompPin = (pt: Point) => {
      for (const other of components) {
        if (other.id === c.id) continue;
        const opins = Components.compPinPositions(other).map(p => ({ x: snapToBaseScalar(p.x), y: snapToBaseScalar(p.y) }));
        for (const op of opins) {
          if (Geometry.eqPtEps(op, pt, 1)) return { other, opins };
        }
      }
      return null;
    };
    ...
    const rightChainToWire = (() => {
      if (!rightCoinc) return false;
      const otherPins = rightCoinc.opins;
      if (otherPins.length < 2) return false;
  ...
  function syncConstraints() {
    ...
    for (const c of components) {
      ...
      let pinOffsets: { x: number, y: number }[] = [];
      try {
      ...
      for (let i = components.indexOf(c) + 1; i < components.length; i++) {
        ...
        try {
          ...
          const nearOnGrid = (pa: Point, pb: Point) => isOnGrid(pa) && isOnGrid(pb) && Math.hypot(pa.x - pb.x, pa.y - pb.y) <= PIN_CONNECT_EPS;
          for (let ia = 0; ia < pinsA.length; ia++) {
            for (let ib = 0; ib < pinsB.length; ib++) {
              const pa = pinsA[ia];
              const pb = pinsB[ib];
              if (!nearOnGrid(pa, pb)) continue;
              constraintSolver.addConstraint({
  ...
  // Find nearest anchor by true (euclidean) distance in screen pixels.
  // Use this for "start from endpoint/pin" behavior where the start must be fully anchored.
  function nearestAnchorToEuclid(pt: Point, thresholdPx = 10) {
    const anchors = collectAnchors();
    const scale = userScale();
    let best: Point | null = null;
    let bestD = Infinity;
    for (const a of anchors) {
      const dx = (a.x - pt.x) * scale;
      const dy = (a.y - pt.y) * scale;
  ...
  function snapPointForTJunction(p: Point, thresholdPx = 10) {
    ...
    if (a) {
      ...
      // If we're close in BOTH axes, treat this like an endpoint/pin snap: anchor exactly.
      // This avoids the start point drifting to the grid when the user is trying to start
      // a new wire from the end of an existing wire.
  ...
  svg.addEventListener('pointerdown', (e) => {
    ...
    let endpointClickedWireId: string | null = null;
    let endpointClickedWireEndIndex: number | null = null;
    if (tgt && (tgt.tagName === 'rect' || tgt.tagName === 'circle') && (tgt as any).endpoint) {
      endpointClicked = (tgt as any).endpoint as Point;
      if (typeof (tgt as any).wireId === 'string') {
        endpointClickedWireId = (tgt as any).wireId as string;
      }
      if (typeof (tgt as any).endpointIndex === 'number') {
        endpointClickedWireEndIndex = (tgt as any).endpointIndex as number;
      }
    }
    ...
    if (mode === 'wire') {
      ...
      if (!drawing.active) {
        // For starting a new wire: if the user is clicking near an existing wire endpoint or pin,
        // anchor the start point EXACTLY to that anchor so it can't drift.
        // Otherwise, fall back to T-junction snapping behavior.
        let startX = x;
        let startY = y;
        if (!endpointClicked) {
          const a = nearestAnchorToEuclid({ x: p.x, y: p.y }, 10);
          if (a) {
            startX = a.x;
            startY = a.y;
          } else {
            const tj = snapPointForTJunction({ x: p.x, y: p.y });
            startX = tj.x;
            startY = tj.y;
          }
        }
        ...
        if ((window as any).routingKernelMode === 'kicad') {
          ...
          // If we started from a wire endpoint, prevent dragging a colinear/coincident
          // first segment along that wire until we move away by at least one snap step.
          // This matches junction-like behavior: you must "pull away" first.
          let beganPlacement = false;
          if (endpointClickedWireId && typeof endpointClickedWireEndIndex === 'number') {
            const w = wires.find(ww => ww.id === endpointClickedWireId);
            if (w && w.points && w.points.length >= 2) {
              const endIdx = Math.max(0, Math.min(w.points.length - 1, endpointClickedWireEndIndex));
              const adjIdx = (endIdx === 0) ? 1 : (endIdx - 1);
              const a = w.points[endIdx];
              const b = w.points[adjIdx];
              const segIsHorizontal = Math.abs(a.y - b.y) < 0.01;
              const segIsVertical = Math.abs(a.x - b.x) < 0.01;
              if (segIsHorizontal) {
                kicadFirstSegmentPerpLock = { lockAxis: 'x', fixed: start.x, minPerpDelta };
                routingFacade.beginPlacement(start, 'VH');
                beganPlacement = true;
              } else if (segIsVertical) {
                kicadFirstSegmentPerpLock = { lockAxis: 'y', fixed: start.y, minPerpDelta };
          ...
          // Fallback: if we didn't click an endpoint marker but still started at a wire endpoint,
          // apply the same protection (best-effort).
          if (!beganPlacement) {
            const ep = findWireEndpointNear(start, 1);
            if (ep && ep.w && ep.w.points && ep.w.points.length >= 2) {
              const endIdx = ep.endIndex;
              const adjIdx = (endIdx === 0) ? 1 : (endIdx - 1);
              const a = ep.w.points[endIdx];
              const b = ep.w.points[adjIdx];
              const segIsHorizontal = Math.abs(a.y - b.y) < 0.01;
              const segIsVertical = Math.abs(a.x - b.x) < 0.01;
              if (segIsHorizontal) {
                kicadFirstSegmentPerpLock = { lockAxis: 'x', fixed: start.x, minPerpDelta };
          ...
          if (beganPlacement) {
            // Placement started; the pointermove handler will enforce the lock.
            return;
          }
  ...
  svg.addEventListener('pointermove', (e) => {
    ...
    if (wireStretchState && mode === 'move') {
      ...
      if (wireStretchState.dragging) {
        ...
        // Free-angle wire drag: translate the entire wire (and any directly connected wire endpoints).
        if (wireStretchState.kind === 'free' && !isHorizontal && !isVertical) {
          const deltaX = p.x - wireStretchState.startMousePos.x;
          const deltaY = p.y - wireStretchState.startMousePos.y;

          // Snap the translation by snapping the first point's target.
          const base = wireStretchState.originalPoints[0] || { x: p0.x, y: p0.y };
          const target = { x: snap(base.x + deltaX), y: snap(base.y + deltaY) };
          const dx = target.x - base.x;
          const dy = target.y - base.y;

          // Move the dragged wire.
          w.points = wireStretchState.originalPoints.map(pt => ({ x: pt.x + dx, y: pt.y + dy }));

          // Rubber-band ghost for endpoints that were stubbed into another wire (T-junction sliding).
          wireStretchState.ghostConnectingWires = [];

          const rbByWhich = new Map<'start' | 'end', (typeof wireStretchState.slidingRubberBands)[number]>();
          for (const rb of wireStretchState.slidingRubberBands) {
            if (!rbByWhich.has(rb.which)) rbByWhich.set(rb.which, rb);
          }
          ...
          for (const stub of wireStretchState.stubbedInto) {
            ...
            const a = host.points[stub.hostSegIndex];
            const b = host.points[stub.hostSegIndex + 1];
            const movedEp = (stub.which === 'start') ? w.points[0] : w.points[w.points.length - 1];
            const forcePerp = (orthoMode || globalShiftDown);
            const EPS = 0.01;
            const isVert = Math.abs(a.x - b.x) < EPS;
            const isHoriz = Math.abs(a.y - b.y) < EPS;
            const hostLen = host.points.length;
            let q: Point;

            // If a rubber-band wire already exists for this endpoint, keep using it.
            // Do NOT create additional perpendicular segments between the rubber-band and the stub.
            const existingRb = rbByWhich.get(stub.which);
            if (existingRb) {
              if (stub.attach === 'endpoint' && stub.hostEndIndex !== undefined) {
                const endIdx = stub.hostEndIndex;
                const hasExplicitJunction = junctions.some(j => Math.abs(j.at.x - host.points[endIdx].x) < 1.0 && Math.abs(j.at.y - host.points[endIdx].y) < 1.0);
                if (!hasExplicitJunction) {
                  if (!forcePerp) {
                    host.points[endIdx] = { x: movedEp.x, y: movedEp.y };
                    updateWireDOM(host);
                    q = { x: movedEp.x, y: movedEp.y };
                  } else {
                    const neighIdx = (endIdx === 0) ? 1 : (endIdx - 1);
                    const neigh = host.points[neighIdx];
                    const ex = host.points[endIdx].x;
                    const ey = host.points[endIdx].y;
                    const segVert = Math.abs(neigh.x - ex) < EPS;
                    const segHoriz = Math.abs(neigh.y - ey) < EPS;
                    if (segVert) {
                      const yRaw = snap(movedEp.y);
                      host.points[endIdx] = { x: ex, y: yRaw };
                      updateWireDOM(host);
                      q = { x: ex, y: yRaw };
                    } else if (segHoriz) {
                      const xRaw = snap(movedEp.x);
                      host.points[endIdx] = { x: xRaw, y: ey };
                      updateWireDOM(host);
                      q = { x: xRaw, y: ey };
                    } else {
                      const vx = neigh.x - ex;
                      ...
                      if (denom < 1e-9) {
                        ...
                        const t = ((movedEp.x - ex) * vx + (movedEp.y - ey) * vy) / denom;
                        q = { x: ex + vx * t, y: ey + vy * t };
                        host.points[endIdx] = { x: q.x, y: q.y };
                        updateWireDOM(host);
                      }
                    }
                  }

                  // Keep the existing rubber-band wire connected: host end follows q.
                  existingRb.wire.points[existingRb.hostEndIndex] = { x: q.x, y: q.y };
                  updateWireDOM(existingRb.wire);
                }
              }
              // Segment-attach case is handled by the existing rubber-band wire update later.
              continue;
            }

            if (stub.attach === 'endpoint' && stub.hostEndIndex !== undefined) {
              // Endpoint-to-endpoint: the stubbed wire controls the host wire endpoint.
              // Keep the host endpoint attached across drags by actively moving it.
              const endIdx = stub.hostEndIndex;
              // If there is an explicit junction dot at this endpoint, treat it as anchored.
              const hasExplicitJunction = junctions.some(j => Math.abs(j.at.x - host.points[endIdx].x) < 1.0 && Math.abs(j.at.y - host.points[endIdx].y) < 1.0);
              if (!hasExplicitJunction) {
                if (!forcePerp) {
                  // Ortho off: coincide with moved endpoint (free stretch).
                  host.points[endIdx] = { x: movedEp.x, y: movedEp.y };
                  updateWireDOM(host);
                  q = { x: movedEp.x, y: movedEp.y };
                } else {
                  // Ortho on: move endpoint along host's adjacent segment direction so the rubber-band is perpendicular.
                  const neighIdx = (endIdx === 0) ? 1 : (endIdx - 1);
                  const neigh = host.points[neighIdx];
                  const ex = host.points[endIdx].x;
                  const ey = host.points[endIdx].y;
                  ...
                  if (segVert) {
                    ...
                    // Diagonal host: move endpoint to the projection on the infinite adjacent segment line.
                    const vx = neigh.x - ex;
                    const vy = neigh.y - ey;
                    const denom = (vx * vx + vy * vy);
                    if (denom < 1e-9) {
                ...
                q = { x: host.points[endIdx].x, y: host.points[endIdx].y };
              }
              // Visual rubber-band from the host endpoint to the moved endpoint (may be zero-length when coincident).
              wireStretchState.ghostConnectingWires.push({ from: { x: q.x, y: q.y }, to: { x: movedEp.x, y: movedEp.y } });
              continue;
            }
            if (forcePerp) {
              // Ortho active: keep rubber-band segment perpendicular to the host wire.
              if (isVert) {
                const yRaw = snap(movedEp.y);
                const minY = Math.min(a.y, b.y);
                const maxY = Math.max(a.y, b.y);
                const minIdx = (a.y <= b.y) ? stub.hostSegIndex : (stub.hostSegIndex + 1);
                const maxIdx = (a.y <= b.y) ? (stub.hostSegIndex + 1) : stub.hostSegIndex;
                const canExtendMin = (minIdx === 0) || (minIdx === hostLen - 1);
                const canExtendMax = (maxIdx === 0) || (maxIdx === hostLen - 1);

                if (yRaw < minY && canExtendMin) {
                  host.points[minIdx] = { x: host.points[minIdx].x, y: yRaw };
                  updateWireDOM(host);
                } else if (yRaw > maxY && canExtendMax) {
                  host.points[maxIdx] = { x: host.points[maxIdx].x, y: yRaw };
                  updateWireDOM(host);
                }
                q = { x: a.x, y: yRaw };
              } else if (isHoriz) {
                ...
                const minX = Math.min(a.x, b.x);
                const maxX = Math.max(a.x, b.x);
                const minIdx = (a.x <= b.x) ? stub.hostSegIndex : (stub.hostSegIndex + 1);
                const maxIdx = (a.x <= b.x) ? (stub.hostSegIndex + 1) : stub.hostSegIndex;
                const canExtendMin = (minIdx === 0) || (minIdx === hostLen - 1);
                const canExtendMax = (maxIdx === 0) || (maxIdx === hostLen - 1);

                if (xRaw < minX && canExtendMin) {
                  host.points[minIdx] = { x: xRaw, y: host.points[minIdx].y };
                  updateWireDOM(host);
                } else if (xRaw > maxX && canExtendMax) {
                  host.points[maxIdx] = { x: xRaw, y: host.points[maxIdx].y };
                  updateWireDOM(host);
                }
                q = { x: xRaw, y: a.y };
              } else {
                // Diagonal host: perpendicular attach is projection onto the infinite line.
                // If projection goes beyond the segment and this segment touches a host endpoint,
                // extend the host wire by moving that endpoint.
                const vx = b.x - a.x;
                const vy = b.y - a.y;
                const denom = (vx * vx + vy * vy);
                if (denom < 1e-9) {
                  ...
                  const t = ((movedEp.x - a.x) * vx + (movedEp.y - a.y) * vy) / denom;
                  q = { x: a.x + vx * t, y: a.y + vy * t };
                  if (t < 0 && stub.hostSegIndex === 0) {
                    host.points[0] = { x: q.x, y: q.y };
                    updateWireDOM(host);
                  } else if (t > 1 && stub.hostSegIndex === hostLen - 2) {
                    host.points[hostLen - 1] = { x: q.x, y: q.y };
                    updateWireDOM(host);
                  }
                }
              }
            } else {
              // Ortho off: let the rubber-band stretch freely (non-perpendicular). The junction point
              // still slides along the host wire based on the dragged wire's translation.
              // Ortho off requirement: keep the connection point on the host anchored in place.
              q = { x: stub.hostPt.x, y: stub.hostPt.y };
            }
            wireStretchState.ghostConnectingWires.push({ from: { x: q.x, y: q.y }, to: { x: movedEp.x, y: movedEp.y } });
          }
          ...
          for (const conn of wireStretchState.connectedWiresStart) {
            const endToUpdate = conn.isStart ? 0 : (conn.wire.points.length - 1);
            conn.wire.points[endToUpdate] = { x: newP0.x, y: newP0.y };
            updateWireDOM(conn.wire);
          }
          for (const conn of wireStretchState.connectedWiresEnd) {
            const endToUpdate = conn.isStart ? 0 : (conn.wire.points.length - 1);
            conn.wire.points[endToUpdate] = { x: newP1.x, y: newP1.y };
            updateWireDOM(conn.wire);
          }

          // If we already have rubber-band wires from previous moves, keep their host-side endpoints
          // sliding along the host wire segment so the junction doesn't become anchored after the first drag.
          // In Ortho mode, the rubber-band segment stays perpendicular to the host.
          if (wireStretchState.slidingRubberBands.length > 0) {
            ...
            for (const rb of wireStretchState.slidingRubberBands) {
              ...
              const a = host.points[rb.hostSegIndex];
              const b = host.points[rb.hostSegIndex + 1];
              const movedEp = rb.wire.points[rb.movingEndIndex];
              ...
              if (forcePerp) {
                if (isVert) {
                  ...
                  const minY = Math.min(a.y, b.y);
                  const maxY = Math.max(a.y, b.y);
                  const minIdx = (a.y <= b.y) ? rb.hostSegIndex : (rb.hostSegIndex + 1);
                  const maxIdx = (a.y <= b.y) ? (rb.hostSegIndex + 1) : rb.hostSegIndex;
                  const canExtendMin = (minIdx === 0) || (minIdx === hostLen - 1);
                  const canExtendMax = (maxIdx === 0) || (maxIdx === hostLen - 1);
                  if (yRaw < minY && canExtendMin) {
                    host.points[minIdx] = { x: host.points[minIdx].x, y: yRaw };
                  ...
                  const minX = Math.min(a.x, b.x);
                  const maxX = Math.max(a.x, b.x);
                  const minIdx = (a.x <= b.x) ? rb.hostSegIndex : (rb.hostSegIndex + 1);
                  const maxIdx = (a.x <= b.x) ? (rb.hostSegIndex + 1) : rb.hostSegIndex;
                  const canExtendMin = (minIdx === 0) || (minIdx === hostLen - 1);
                  const canExtendMax = (maxIdx === 0) || (maxIdx === hostLen - 1);
                  if (xRaw < minX && canExtendMin) {
                    host.points[minIdx] = { x: xRaw, y: host.points[minIdx].y };
                  ...
                  if (denom < 1e-9) {
                    ...
                    const t = ((movedEp.x - a.x) * vx + (movedEp.y - a.y) * vy) / denom;
                    q = { x: a.x + vx * t, y: a.y + vy * t };
                    if (t < 0 && rb.hostSegIndex === 0) {
                      host.points[0] = { x: q.x, y: q.y };
                      updateWireDOM(host);
                    } else if (t > 1 && rb.hostSegIndex === hostLen - 2) {
                      host.points[hostLen - 1] = { x: q.x, y: q.y };
                      updateWireDOM(host);
                    }
                  }
                ...
                // Ortho off: keep a sliding attach point that follows translation along the host.
                // Ortho off requirement: keep the host-side connection point anchored.
                q = { x: rb.hostPt.x, y: rb.hostPt.y };
              }

              rb.wire.points[rb.hostEndIndex] = { x: q.x, y: q.y };
              updateWireDOM(rb.wire);
            }
          }
    ...
    if (mode === 'wire' && drawing.active) {
      if ((window as any).routingKernelMode === 'kicad') {
        ...
        // Tracking/connection hints (legacy-style) for KiCad routing: while dragging the wire end,
        // detect alignment with endpoints of other wires along X or Y and draw a hint line.
        kicadTrackingHint = null;
        if (trackingMode && drawing.active && drawing.cursor) {
          const scale = svg.clientWidth / Math.max(1, viewW);
          const snapTol = HINT_SNAP_TOLERANCE_PX / scale;

          const rawX = p.x;
          const rawY = p.y;

          // Determine the direction of the *current* segment being dragged.
          // Hints should be orthogonal to this direction (legacy behavior):
          // - If dragging mostly horizontally, only consider X-alignment (vertical hint line)
          // - If dragging mostly vertically, only consider Y-alignment (horizontal hint line)
          let segStart: Point | null = null;
          if (drawing.points && drawing.points.length >= 2) {
            segStart = drawing.points[drawing.points.length - 2];
          }
          const dxDrag = segStart ? (drawing.cursor.x - segStart.x) : 0;
          ...
          const considerCandidate = (cand: Point) => {
            const k = Geometry.keyPt({ x: Math.round(cand.x), y: Math.round(cand.y) });
            if (excludeKeys.has(k)) return;

            if (mostlyHorizontal) {
              // Only X alignment => vertical hint line, orthogonal to horizontal drag.
              const xDist = Math.abs(rawX - cand.x);
              if (xDist < snapTol && xDist < bestAxisDist) { bestAxisDist = xDist; bestCand = cand; }
            } else {
              // Only Y alignment => horizontal hint line, orthogonal to vertical drag.
              const yDist = Math.abs(rawY - cand.y);
              if (yDist < snapTol && yDist < bestAxisDist) { bestAxisDist = yDist; bestCand = cand; }
            }
          };
          ...
          for (const w of wires) {
            ...
            considerCandidate(firstPt);
            considerCandidate(lastPt);
          }

          // Also allow corners from earlier segments in the in-progress wire being placed.
          if (drawing.points && drawing.points.length >= 3) {
            for (let i = 0; i < drawing.points.length - 2; i++) {
              considerCandidate(drawing.points[i]);
            }
          }
          ...
          wireStretchState.ghostConnectingWires.forEach(ghost => {
            ...
            if (dist > 5) {
              const newWire: Wire = {
                ...
                netId: wireStretchState.wire.netId
              };
              wires.push(newWire);
  ...
  function renderConnectionHint() {
    ...
    if (kicadTrackingHint && drawing.active) {
      const hintLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      hintLine.setAttribute('data-hint', '1');
      hintLine.setAttribute('stroke', '#00ff00'); // bright green
    ...
    // While dragging a non-orthogonal wire with Tracking enabled, highlight when the
    // current segment is collinear with an existing wire segment at the startpoint.
    if (trackingMode && drawing.active) {
      const currentDragSegment = () => {
        if (drawing.cursor && drawing.points && drawing.points.length > 0) {
          const a0 = drawing.points[drawing.points.length - 1];
          const b0 = drawing.cursor;
          const d = Math.hypot(b0.x - a0.x, b0.y - a0.y);
          if (d > 1e-6) return { a: a0, b: b0 };
        }
        if (drawing.points && drawing.points.length >= 2) {
          return { a: drawing.points[drawing.points.length - 2], b: drawing.points[drawing.points.length - 1] };
        }
        return null;
      };
      ...
      const areOppositeCollinear = (da, db, eps = 1e-2) => {
        ...
        if (Math.abs(cross) > eps * na * nb) return false;
        const dot = da.dx * db.dx + da.dy * db.dy;
        return dot < 0;
      };

      const seg = currentDragSegment();
      if (seg) {
        const a = { x: snapToBaseScalar(seg.a.x), y: snapToBaseScalar(seg.a.y) };
        const b = { x: snapToBaseScalar(seg.b.x), y: snapToBaseScalar(seg.b.y) };
        const isNonOrtho = !(a.x === b.x || a.y === b.y);
        if (isNonOrtho) {
          const startKey = Geometry.keyPt({ x: Math.round(a.x), y: Math.round(a.y) });
          const newDir = { dx: b.x - a.x, dy: b.y - a.y };

          let highlightSeg: { p0: Point; p1: Point } | null = null;
          for (const w of wires) {
            if (!w.points || w.points.length < 2) continue;
            const pStart0 = w.points[0];
            const pEnd0 = w.points[w.points.length - 1];
            const key0 = Geometry.keyPt({ x: Math.round(pStart0.x), y: Math.round(pStart0.y) });
            const key1 = Geometry.keyPt({ x: Math.round(pEnd0.x), y: Math.round(pEnd0.y) });
            if (key0 !== startKey && key1 !== startKey) continue;

            const endIndex = (key0 === startKey) ? 0 : (w.points.length - 1);
            const other0 = (endIndex === 0) ? w.points[1] : w.points[w.points.length - 2];
            const p0 = { x: snapToBaseScalar(w.points[endIndex].x), y: snapToBaseScalar(w.points[endIndex].y) };
            const p1 = { x: snapToBaseScalar(other0.x), y: snapToBaseScalar(other0.y) };
            const existingDir = { dx: p1.x - p0.x, dy: p1.y - p0.y };

            if (areOppositeCollinear(existingDir, newDir)) {
              highlightSeg = { p0, p1 };
              break;
            }
          }
          ...
          if (highlightSeg) {
            ...
            hl.setAttribute('data-hint', '1');
            hl.setAttribute('stroke', '#00ff00');
            hl.setAttribute('stroke-width', '3');
            hl.setAttribute('stroke-linecap', 'round');
            hl.setAttribute('pointer-events', 'none');
            hl.setAttribute('opacity', '0.9');
            hl.setAttribute('x1', String(highlightSeg.p0.x));
            hl.setAttribute('y1', String(highlightSeg.p0.y));
            hl.setAttribute('x2', String(highlightSeg.p1.x));
            hl.setAttribute('y2', String(highlightSeg.p1.y));
            gOverlay.appendChild(hl);
          }
        }
      }
    }
  ...
  function buildWireStrokeMenu(menuEl: HTMLElement) {
    ...
    try { chkCustom.name = `app-chk-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
    ...
    try { inpW.name = `app-dim-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
    ...
    try { selS.name = `app-select-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
    ...
    inpColor.value = hex;
    try { inpColor.name = `app-color-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
    const inpA = document.createElement('input'); inpA.type = 'range'; inpA.min = '0'; inpA.max = '1'; inpA.step = '0.01'; inpA.value = String(rgb.a ?? 1);
    try { inpA.name = `app-range-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
  ...
  function dimNumberPx(pxVal: number, onCommit: (px: number) => void) {
    ...
    try { inp.name = `app-dim-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
    return inp;
  }

  // Forward declarations for UI updates on unit change
  let updateJunctionSizeUI: (() => void) | null = null;
  let updateComponentClearanceUI: (() => void) | null = null;
  ...
  function setGlobalUnits(u: 'mm' | 'in' | 'mils') {
    ...
    if (updateComponentClearanceUI) updateComponentClearanceUI();
  ...
  (function installUnitsSelect() {
    ...
    try {
      ...
      if (injectionTarget) {
        ...
        const inp = dimNumberPx(getPx(), (px: number) => {
        ...
        try { lbl.htmlFor = inp.id; } catch (_) {}
  ...
  function axisAtEndpoint(w, endIndex) {
    ...
    if (a && b && a.y === b.y) return 'x';
    if (a && b && a.x === b.x) return 'y';
    return null;
  }
  function dirAtEndpoint(w, endIndex) {
    const n = w.points.length; if (n < 2) return null;
  ...
  function endpointPairsByKey() {
    // key -> array of { w, endIndex, dir, other }
    const map = new Map();
    for (const w of wires) {
      const n = w.points.length;
      if (n < 2) continue;
      ...
      for (const endIndex of ends) {
        ...
        const dir = dirAtEndpoint(w, endIndex);
        const other = (endIndex === 0) ? w.points[1] : w.points[n - 2];
        (map.get(key) || (map.set(key, []), map.get(key))).push({ w, endIndex, dir, other });
      }
    }
    return map;
  }
        ...
        if (edge && edge.wireId) {
          ...
          if (w) {
            ...
            if (edge.axis !== 'x' && edge.axis !== 'y') {
              ...
              let ax: 'x' | 'y' | null = null;
              const n = w.points.length;
              if (n >= 2) {
                let other: { x: number; y: number } | null = null;
                if (isStart) other = w.points[1];
                else if (isEnd) other = w.points[n - 2];
                else if (n === 2) other = w.points[1];
                if (other) {
                  if (Math.round(other.y) === node.y) ax = 'x';
                  else if (Math.round(other.x) === node.x) ax = 'y';
                }
              }
              if (ax) axesAtNode.add(ax);
            }
          }
        }
      }
