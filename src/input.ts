...
export interface InputContext {
  ...
  panStartView: { x: number; y: number };
  panPointerId: number | null;
  panAnimationFrame: number | null;
  pendingPanPosition: { x: number; y: number } | null;
  ...
  svg: SVGSVGElement;
  gComps: SVGGElement;
  gOverlay: SVGGElement;
  gDrawing: SVGGElement;
  
  // UI elements
  coordDisplay: HTMLElement | null;
  coordInputX: HTMLInputElement | null;
  coordInputY: HTMLInputElement | null;
  coordInputGroup: HTMLElement | null;
  coordInputLength: HTMLInputElement | null;
  coordInputAngle: HTMLInputElement | null;
  polarInputGroup: HTMLElement | null;
  
  // Helpers
  $qa: <T extends Element>(sel: string, parent?: Element) => T[];
  svgPoint: (evt: any) => Point;
  snap: (v: number) => number;
  snapToBaseScalar: (v: number) => number;
  snapPointPreferAnchor: (p: Point, thresholdPx?: number) => Point;
  nearestSegmentAtPoint: (p: Point, thresholdPx: number) => any;
  ...
  redrawGrid: () => void;
  renderDrawing: () => void;
  renderCrosshair: (x: number, y: number) => void;
  clearCrosshair: () => void;
  renderConnectionHint: () => void;
  renderInspector: () => void;
  renderGhostAt: (pos: Point, type: PlaceType) => void;
  clearGhost: () => void;
  finishWire: () => void;
  pushUndo: () => void;
  undo: () => void;
  ...
  removeJunctionsAtWireEndpoints: (w: Wire) => void;
  ...
  isTwoPinType: (type: PlaceType) => boolean;
  ...
  updateCoordinateInputs: (x: number, y: number) => void;
  showCoordinateInputs: () => void;
  hideCoordinateInputs: () => void;
  updatePolarInputs: (x: number, y: number) => void;
  showPolarInputs: () => void;
  hidePolarInputs: () => void;
  isEditingKeystrokesTarget: (e: KeyboardEvent) => boolean;
  rebuildTopology: () => void;
  debugDumpAnchors: () => void;
  updateSubtypeVisibility: () => void;
  updateCapacitorButtonIcon: () => void;
  updateCapacitorSubtypeButtons: () => void;
  
  // Constants
  GRID: number;
  HINT_SNAP_TOLERANCE_PX: number;
  HINT_UNLOCK_THRESHOLD_PX: number;
}

// Marquee selection helpers
export function beginMarqueeAt(ctx: InputContext, p: Point, startedOnEmpty: boolean, preferComponents: boolean): void {
  ...
  if (ctx.marquee.rectEl) ctx.marquee.rectEl.remove();
...
export function updateMarqueeTo(ctx: InputContext, p: Point): void {
  if (!ctx.marquee.active || !ctx.marquee.start || !ctx.marquee.rectEl) return;
  ctx.marquee.end = p;
 ...
 */
export function updateMarqueeStroke(marqueeEl: SVGRectElement | null, zoom: number): void {
  if (!marqueeEl) return;
...
export function finishMarquee(ctx: InputContext): boolean {
  if (!ctx.marquee.active || !ctx.marquee.start || !ctx.marquee.end) return false;
  ...
  const inRect = (p: Point, rect: { x: number; y: number; w: number; h: number }): boolean => {
    return p.x >= rect.x && p.x <= rect.x + rect.w && p.y >= rect.y && p.y <= rect.y + rect.h;
  };
  
  // Helper: check if segment intersects rect
  const segmentIntersectsRect = (a: Point, b: Point, rect: { x: number; y: number; w: number; h: number }): boolean => {
    if (inRect(a, rect) || inRect(b, rect)) return true;
    const left = rect.x, right = rect.x + rect.w, top = rect.y, bottom = rect.y + rect.h;
    const lineIntersectsEdge = (p1: Point, p2: Point, e1: Point, e2: Point): boolean => {
  ...
  const segs: Array<{ w: Wire; idx: number; d2: number }> = [];
  ...
  const comps: Array<{ c: Component; d2: number }> = [];
...
// Pan helpers
export function beginPan(ctx: InputContext, e: PointerEvent): void {
  ctx.isPanning = true;
...
export function endPan(ctx: InputContext): void {
  if (!ctx.isPanning) return;
  ctx.isPanning = false;
...
// Zoom helper
export function handleWheel(ctx: InputContext, e: WheelEvent): void {
  e.preventDefault();
...
export function updateCoordinateDisplay(ctx: InputContext, x: number, y: number): void {
  ctx.updateCoordinateDisplay(x, y);
}

export function hideCoordinateDisplay(ctx: InputContext): void {
  ctx.hideCoordinateDisplay();
}

export function updateCoordinateInputs(ctx: InputContext, x: number, y: number): void {
  lastMouseX = x;
  lastMouseY = y;
  ctx.updateCoordinateInputs(x, y);
}

export function showCoordinateInputs(ctx: InputContext): void {
  ctx.showCoordinateInputs();
}

export function hideCoordinateInputs(ctx: InputContext): void {
  ctx.hideCoordinateInputs();
  coordInputActive = false;
}

export function updatePolarInputs(ctx: InputContext, x: number, y: number): void {
  ctx.updatePolarInputs(x, y);
}

export function showPolarInputs(ctx: InputContext): void {
  ctx.showPolarInputs();
}

export function hidePolarInputs(ctx: InputContext): void {
  ctx.hidePolarInputs();
  coordInputActive = false;
}

// Pointer event handlers
export function handlePointerDown(ctx: InputContext, e: PointerEvent): void {
...
export function handlePointerMove(ctx: InputContext, e: PointerEvent): void {
  if (ctx.isPanning) {
    doPan(ctx, e);
    return;
  }
  ...
  if (ctx.mode === 'wire' && ctx.drawing.active) {
    ...
    if (ctx.shiftOrthoVisualActive) {
      ...
      if (ctx.updateOrthoButtonVisual) ctx.updateOrthoButtonVisual();
    }
  }
...
export function handlePointerUp(ctx: InputContext, e: PointerEvent): void {
  if (ctx.marquee.active) {
    finishMarquee(ctx);
  }
  endPan(ctx);
...
export function handlePointerLeave(ctx: InputContext, e: PointerEvent): void {
  endPan(ctx);
}

export function handleDoubleClick(ctx: InputContext, e: MouseEvent): void {
  if (ctx.mode === 'wire' && ctx.drawing.active) {
    ctx.finishWire();
  }
}
...
export function handleKeyDown(ctx: InputContext, e: KeyboardEvent): void {
  ...
  // Mode shortcuts
  if (e.key.toLowerCase() === 'w') ctx.setMode('wire');
  if (e.key.toLowerCase() === 'v') ctx.setMode('select');
  if (e.key.toLowerCase() === 'p') ctx.setMode('pan');
  if (e.key.toLowerCase() === 'm') ctx.setMode('move');
...
function handlePlaceJunction(ctx: InputContext, p: Point, x: number, y: number): void {
  const TOL = 50;
...
function handleDeleteJunction(ctx: InputContext, p: Point): void {
  const TOL = 50;
  const tol = TOL * 0.0254 * (100 / 25.4);
  let idx = -1;
...
function handlePlaceComponent(ctx: InputContext, x: number, y: number, p: Point): void {
  ...
  if (ctx.isTwoPinType(ctx.placeType!)) {
    ...
    if (hit) {
      at = hit.q;
      const normDeg = (angle: number) => ((angle % 360) + 360) % 360;
...
function handleWireClick(ctx: InputContext, x: number, y: number, e: PointerEvent): void {
  if (!ctx.drawing.active) {
    ...
    if (!endpointData && tgt) {
      const allRects = [
        ...ctx.$qa<SVGRectElement>('rect[data-endpoint]', ctx.gOverlay),
        ...ctx.$qa<SVGRectElement>('rect', ctx.gDrawing)
      ];
...
function handleWireMovement(ctx: InputContext, e: PointerEvent, p: Point, x: number, y: number): void {
...
function handleConnectionHint(ctx: InputContext, p: Point, x: number, y: number, last: Point, dx: number, dy: number, forceOrtho: boolean): void {
  ...
  ctx.wires.forEach(w => {
    if (w.points && w.points.length >= 2) {
      ...
      if (!isDrawingStart(firstPt)) candidates.push(firstPt);
      const lastPt = w.points[w.points.length - 1];
      if (!isDrawingStart(lastPt)) candidates.push(lastPt);
    }
  });
  ...
  ctx.components.forEach(c => {
    ...
    pins.forEach(p => {
      if (!isDrawingStart(p)) candidates.push({ x: p.x, y: p.y });
    });
  });
  ...
  if (ctx.connectionHint) {
    const distFromTarget = Math.sqrt(
      Math.pow(x - ctx.connectionHint.targetPt.x, 2) +
  ...
  if (!ctx.connectionHint && candidates.length > 0) {
    ...
    const shouldExcludeCandidate = (cand: Point, isHorizontalHint: boolean): boolean => {
      ...
      if (isDraggingVertically && hintIsVertical) return true;
      if (!isDraggingVertically && !hintIsVertical) return true;
...
export function installInputHandlers(ctx: InputContext): void {
  // Global shift tracking
  window.addEventListener('keydown', (e) => { if (e.key === 'Shift') ctx.globalShiftDown = true; });
  window.addEventListener('keyup', (e) => { if (e.key === 'Shift') ctx.globalShiftDown = false; });
