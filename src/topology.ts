...
import { Point, Wire, Junction, Component, Stroke } from './types.js';
...
export interface SWP {
  ...
  color: string;
  edgeWireIds: string[];
  edgeIndicesByWire: Record<string, number[]>;
}
...
export function rebuildTopology(
  ...
  const axisOf = (a: Point, b: Point): 'x' | 'y' | null => 
    (a.y === b.y) ? 'x' : (a.x === b.x) ? 'y' : null;
  
  function addNode(p: Point): string {
    const k = keyPt(p);
    if (!nodes.has(k)) {
      nodes.set(k, { 
        x: Math.round(p.x), 
  ...
  let segments: Array<{ w: Wire, i: number, a: Point, b: Point }> = [];
  ...
  function isEndpoint(pt: Point, seg: { a: Point, b: Point }): boolean {
    return (pt.x === seg.a.x && pt.y === seg.a.y) || (pt.x === seg.b.x && pt.y === seg.b.y);
  }

  let intersectionPoints: string[] = [];
  ...
  const insertMap = new Map<string, Point[]>();
  for (let i = 0; i < segments.length; i++) {
    const s = segments[i];
  ...
  for (const [wid, ptsToInsert] of insertMap.entries()) {
    ...
    for (let i = 1; i < w.points.length; i++) {
      ...
      let segPts = ptsToInsert.filter(pt => {
        if (a.x === b.x && pt.x === a.x && 
            Math.min(a.y, b.y) < pt.y && pt.y < Math.max(a.y, b.y)) return true;
        if (a.y === b.y && pt.y === a.y && 
            Math.min(a.x, b.x) < pt.x && pt.x < Math.max(a.x, b.x)) return true;
        return false;
      });
    ...
    // Remove duplicates
    w.points = newPts.filter((pt, idx, arr) => 
      idx === 0 || pt.x !== arr[idx - 1].x || pt.y !== arr[idx - 1].y
    );
  }
  ...
  for (const w of wires) {
    ...
    for (let i = 1; i < pts.length; i++) {
      ...
      for (const p of segIntersections) segPts.push(p);
      segPts.push(b);
    }
  ...
  const updatedJunctions = detectJunctions(
    nodes, 
    edges, 
    wires, 
    components, 
...
function buildSWPs(
  ...
  function otherEdgeWithSameAxis(
    nodeKey: string, 
    fromEdge: TopologyEdge
  ): TopologyEdge | null {
  ...
  for (const e0 of edges) {
    ...
    function walkDir(cur: TopologyEdge | null, enterNodeKey: string) {
    ...
    let start: TopologyNode, end: TopologyNode;
    const axis = e0.axis;
    ...
    if (axis === 'x') {
      leadEdge = chain.reduce((m, e) => 
        Math.min(e.a.x, e.b.x) < Math.min(m.a.x, m.b.x) ? e : m, chain[0]
      );
    } else {
      leadEdge = chain.reduce((m, e) => 
        Math.min(e.a.y, e.b.y) < Math.min(m.a.y, m.b.y) ? e : m, chain[0]
      );
    }
    ...
    const edgeWireIds = [...new Set(chain.map(e => e.wireId).filter(Boolean) as string[])];
    const edgeIndicesByWire: Record<string, number[]> = {};
    for (const e of chain) {
      if (!e.wireId) continue;
      if (!edgeIndicesByWire[e.wireId]) edgeIndicesByWire[e.wireId] = [];
 ...
 */
function mapComponentsToSWPs(
  components: Component[], 
  compPinPositions: (c: Component) => Point[], 
  swps: SWP[]
): Map<string, string> {
  const compToSwp = new Map<string, string>();
...
function detectJunctions(
  ...
  const newJunctions: Junction[] = [...manualJunctions];
 ...
 * Find SWP by ID.
 */
export function findSwpById(topology: Topology, id: string): SWP | undefined {
 ...
 */
export function swpIdForComponent(topology: Topology, componentId: string): string | null {
 ...
 */
export function swpForWireSegment(topology: Topology, wireId: string): SWP | null {
