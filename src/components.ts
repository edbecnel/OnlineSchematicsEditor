 ...
 */
export function compPinPositions(c: Component): Array<{
  x: number;
  y: number;
  name: string;
  id: string;
 ...
 */
export function formatValue(c: Component): string {
  if (!c.value) return '';
 ...
 */
export function isTwoPinType(type: string): boolean {
 ...
 */
export function getDefaultLabel(type: string, counter: number): string {
 ...
 */
export function getDiodeSubtypeName(subtype: DiodeSubtype): string {
  const names: Record<DiodeSubtype, string> = {
 ...
 */
export function getDiodeSubtypes(): DiodeSubtype[] {
  return [
    'generic',
    'schottky',
    'zener',
