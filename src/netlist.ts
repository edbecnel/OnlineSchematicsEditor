...
import { Wire, NetClass, RGBA01, StrokeType } from './types.js';
 ...
 */
export function netClassForWire(
  w: Wire,
 ...
 */
export function createDefaultNetClasses(
  defaultWireColor: string,
  cssToRGBA01: (css: string) => RGBA01
): Record<string, NetClass> {
  return {
    default: {
      id: 'default',
      name: 'Default',
...
export type WireColorMode = 'custom' | 'auto' | 'white' | 'black' | 'red' | 'green' | 'blue' | 'yellow' | 'magenta' | 'cyan';

/**
 * Resolve wire color from color mode.
 */
export function resolveWireColor(mode: WireColorMode): string {
 ...
 */
export function renderNetList(
  nets: Set<string>,
  wires: Wire[],
  activeNetClass: string,
  NET_CLASSES: Record<string, NetClass>,
  onSetActive: (netName: string) => void,
  onEditNet: (netName: string) => void,
  onDeleteNet: (netName: string) => void
): void {
...
export function deleteNet(
  ...
  if (netName === 'default') return false;
  ...
  wires.forEach(w => { 
    if (w.netId === netName) w.netId = 'default'; 
  });
...
export interface NetPropertiesDialogOptions {
  netName: string;
  netClass: NetClass;
  globalUnits: 'mm' | 'in' | 'mils';
  NM_PER_MM: number;
  formatDimForDisplay: (nm: number, units: string) => string;
  parseDimInput: (input: string, units: string) => { nm: number } | null;
  colorToHex: (css: string) => string;
  rgba01ToCss: (c: RGBA01) => string;
  onSave: (updates: { width: number; type: StrokeType; color: RGBA01 }) => void;
}

/**
 * Show net properties dialog for editing wire width, style, and color.
 */
export function showNetPropertiesDialog(options: NetPropertiesDialogOptions): void {
  const {
    netName,
    netClass,
    globalUnits,
  ...
  try { widthInput.name = `net-width-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
  ...
  try { styleSelect.name = `net-style-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
  ...
  try { colorInput.name = `net-color-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
  ...
  try { alphaInput.name = `net-alpha-${Math.random().toString(36).slice(2,8)}`; } catch (_) {}
  ...
  saveBtn.onclick = (e) => {
    ...
    onSave({
      width: valMm,
      type: styleSelect.value as StrokeType,
