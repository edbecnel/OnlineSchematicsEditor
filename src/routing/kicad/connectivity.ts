import { type KPoint, type KWire, type KJunction, type KPinRef, type RoutingState, type Connectivity, type NetMember, type DerivedNet } from './model.js';
...
class DisjointSet {
  ...
  union(a: number, b: number) {
    ...
    if (raRank < rbRank) this.parent[ra] = rb;
    else if (raRank > rbRank) this.parent[rb] = ra;
    else { this.parent[rb] = ra; this.rank[ra]++; }
  }
}

function dist2(a: KPoint, b: KPoint): number {
  const dx = a.x - b.x, dy = a.y - b.y;
  return dx * dx + dy * dy;
}

function pointToSegmentDistance(p: KPoint, a: KPoint, b: KPoint): { distance: number; onSegment: boolean } {
...
function keyPt(p: KPoint): string {
  // Use rounded coordinates to keep determinism and dedupe.
  return `${Math.round(p.x)},${Math.round(p.y)}`;
}

export function deriveConnectivity(state: RoutingState): Connectivity {
  const tol = state.tolerance;
  const tol2 = tol * tol;

  // Track derived implicit T-junction points (endpoint-on-segment). Never persist these.
  const implicitJunctionsByKey: Map<string, KPoint> = new Map();

  // Collect node entries: wire endpoints, pins, and junctions
  type Node = { id: string; pos: KPoint; member: NetMember };
  const nodes: Node[] = [];
  const endpointNodeIndex: Map<string, number> = new Map(); // key: wireId:endIdx
  const pinNodeIndex: Map<string, number> = new Map(); // key: pinId
  const junctionNodeIndex: Map<string, number> = new Map(); // key: junctionId

  // Helper to add a node
  function addNode(id: string, pos: KPoint, member: NetMember): number {
  ...
  for (const w of state.wires) {
    ...
    if (i0 !== undefined && i1 !== undefined) dsu.union(i0, i1);
  }

  // Rule: endpoint-to-endpoint connects within tolerance
  for (let i = 0; i < nodes.length; i++) {
    const ni = nodes[i];
    if (ni.member.kind !== 'wire-endpoint') continue;
    for (let j = i + 1; j < nodes.length; j++) {
      const nj = nodes[j];
      if (nj.member.kind !== 'wire-endpoint') continue;
      if (dist2(ni.pos, nj.pos) <= tol2) dsu.union(i, j);
    }
  }
  ...
  for (const [pid, pIdx] of pinNodeIndex.entries()) {
    ...
    for (const [eKey, eIdx] of endpointNodeIndex.entries()) {
      ...
      if (dist2(pNode.pos, eNode.pos) <= tol2) dsu.union(pIdx, eIdx);
    }
  }
  ...
  for (const wEnd of state.wires) {
    if (!wEnd.points || wEnd.points.length < 2) continue;
    const endpoints: Array<{ idx: 0 | 1; pos: KPoint }> = [
  ...
  for (const j of state.junctions) {
    ...
    for (const [_, eIdx] of endpointNodeIndex.entries()) {
      if (dist2(nodes[eIdx].pos, j.at) <= tol2) dsu.union(jIdx, eIdx);
    }
    for (const w of state.wires) {
      ...
      for (let s = 0; s < pts.length - 1; s++) {
        ...
        if (onSegment && distance <= tol) {
          ...
          const i0 = endpointNodeIndex.get(`${w.id}:0`);
          if (i0 !== undefined) dsu.union(jIdx, i0);
          break;
        }
      }
    }
  ...
  const groups: Map<number, DerivedNet> = new Map();
  for (let i = 0; i < nodes.length; i++) {
    const r = dsu.find(i);
