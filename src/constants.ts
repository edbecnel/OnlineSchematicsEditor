// ====== Constants and Configuration ======

// Grid and snapping
export const GRID = 25; // px; 2*GRID = 50 px = exactly 10 snap units (500 mils)

// Nanometer resolution constants (internal units)
export const NM_PER_MM = 1_000_000;   // 1 mm == 1,000,000 nm
export const NM_PER_IN = 25_400_000;  // 25.4 mm == 1 inch
export const NM_PER_MIL = NM_PER_IN / 1000; // 0.001 in

// Snapping resolution: 50 mils (0.05 in) internal nm value
export const SNAP_MILS = 50;
export const SNAP_NM = Math.round(NM_PER_MIL * SNAP_MILS); // nanometers for snap resolution

// Viewport
export const BASE_W = 1600;
export const BASE_H = 1000;

// Connection hints
export const HINT_SNAP_TOLERANCE_PX = 5;  // Direct pixel tolerance for snap detection
export const HINT_UNLOCK_THRESHOLD_PX = 5; // Direct pixel threshold for unlocking

// Unit options for component values
export const UNIT_OPTIONS = {
  resistor: ['Ω', 'kΩ', 'MΩ'],
  capacitor: ['pF', 'nF', 'µF', 'mF'],
  inductor: ['nH', 'µH', 'mH', 'H'],
  default: ['']
};

// Wire color options
export const WIRE_COLOR_OPTIONS: Array<[string, string]> = [
  ['auto', 'Netclass defaults'],
  ['white', 'White'], ['black', 'Black'],
  ['red', 'Red'], ['green', 'Green'], ['blue', 'Blue'],
  ['yellow', 'Yellow'], ['magenta', 'Magenta'], ['cyan', 'Cyan']
];
