 ...
 */
export const keyPt = (p: Point): string => `${Math.round(p.x)},${Math.round(p.y)}`;

/**
 * Compare two numbers with epsilon tolerance
 */
export const eqN = (a: number, b: number, eps = 0.5): boolean => Math.abs(a - b) <= eps;

/**
 * Compare two points with epsilon tolerance
 */
export function eqPtEps(a: Point, b: Point, eps = 0.75): boolean {
 ...
 */
export function samePt(a: Point | null, b: Point | null): boolean {
 ...
 * t = 0 at a, t = 1 at b
 */
export function projectPointToSegmentWithT(p: Point, a: Point, b: Point): { proj: Point; t: number } {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  const lenSq = dx * dx + dy * dy;
  if (lenSq === 0) return { proj: { x: a.x, y: a.y }, t: 0 };
 ...
 */
export function indexOfPointEps(pts: Point[], p: Point, eps = 0.75): number {
  for (let i = 0; i < pts.length; i++) {
    if (eqPtEps(pts[i], p, eps)) return i;
  }
  return -1;
}
 ...
 */
export function orderPointsEndingAt(pts: Point[], pin: Point): Point[] {
  const n = pts.length;
 ...
 */
export function orderPointsStartingAt(pts: Point[], pin: Point): Point[] {
  const n = pts.length;
  if (n === 0) return pts.slice();
 ...
 */
export function normalizedPolylineOrNull(pts: Point[] | undefined): Point[] | null {
 ...
 */
export function nearestSegmentIndex(pts: Point[], p: Point): { index: number; dist: number; proj: Point } | null {
  if (pts.length < 2) return null;
  let bestIdx = 0;
  let bestDist = Infinity;
  let bestProj: Point = pts[0];
 ...
 */
export function midOfSeg(pts: Point[], idx: number): Point {
  const a = pts[idx];
  const b = pts[idx + 1];
 ...
 */
export function axisAlignedIntersection(
  a1: Point, a2: Point,
  b1: Point, b2: Point
): Point | null {
